...
struct EntityPoolTask
{
    ...
    inline bool CheckEntity(uintptr_t address)
    {
        ...
        {
            float position[3];
            GTAmemory::GetEntityPos(
                address, position
            ); // replaced _getEntityPos with a call to GetEntityPos which implements the same function for enhanced (because it was inlined), and calls the old function for legacy.
...
static bool IsPtfxEntityUsable(uint64_t entity)
{
    ...
    if (!s_isPtfxEntityUsableVFunc || s_ptfxEntityVPtr == 0)
    {
        ...
        s_isPtfxEntityUsableVFunc = reinterpret_cast<uint8_t (*)(uint64_t, uint64_t, uint64_t)>(funcPtr);
    }
...
void GTAmemory::Init()
{
    ...
    if (!g_isEnhanced)
    {
        ...
        address =
            (uintptr_t)MemryScan::PatternScanner::FindPattern("48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 48 89 78 20 41 56 48 83 EC 20 33 DB 41 8B F0 8B EA 48 8B F9 66 3B 59 08 73 32 48 8B 0F");
        if (!address)
        {
    ...
    if (g_isEnhanced)
    {
        ...
        if (address)
        {
            ...
            if (address)
            {
                ...
                address = MemryScan::PatternScanner::FindPattern(
                    "\x3B\x05\x00\x00\x00\x00\x7D\x00\x48\x8B\x0D", "xx????x?xxx", address, 200
                ); // TODO: use the findpattern with legacy patterns, because it supports a start address.
                if (address)
                {
...
INT32 GTAmemory::TransitionStatus()
{
    ...
    case eGameVersion::VER_1_0_1365_1_STEAM:
    case eGameVersion::VER_1_0_1365_1_NOSTEAM:
        _transitionStatus = GTAmemory::GetGlobalPtr<INT32>(1312789);
        break;
    case eGameVersion::VER_1_0_1604_1_STEAM:
    case eGameVersion::VER_1_0_1604_1_NOSTEAM:
        _transitionStatus = GTAmemory::GetGlobalPtr<INT32>(1312792);
        break;
    }
...
void SpSnow::EnableSnow(bool bEnable)
{
    ...
    static auto SpSnow_addr2 =
        MemryScan::PatternScanner::FindPattern("44 38 3D ? ? ? ? 74 0F 83 3F"); // 80 3d ? ? ? ? ? 8b 86 ? ? ? ? 74 find out what is being patched in legacy and do the same here.
    ...
    static auto SpSnow_addr22 =
        MemryScan::PatternScanner::FindPattern("44 38 3D ? ? ? ? 74 1D B9 40 00 00 00"); // eb ? 80 3d ? ? ? ? ? 74 ? b9 ? ? ? ? e8 ? ? ? ? 84 c3 74 ? c7 86 its somewhere here. find it.
    ...
    if (!this->bInitialized)
    {
        if (g_isEnhanced)
        {
            ...
            if (this->traxv_call == NULL)
                this->tracks_available = false;
...
SpSnow::SpSnow()
    : bDisabled(true), loop_spsnow(false), bInitialized(false), bUseAddr13(false), bUseAddr22(false), tracks_available(true), traxp(NULL), traxp_call(NULL), traxpt(NULL), traxv(NULL),
      traxv_call(NULL), traxvt(NULL)
{
}
...
void GeneralGlobalHax::DisableAnnoyingRecordingUI(bool uSure)
{
    ...
    else
    {
        ...
        case eGameVersion::VER_1_0_791_2_STEAM:
            *GTAmemory::GetGlobalPtr<INT32>(0x42DE + 0x9) = uSure ? 1 : 0;
            break;
...
std::string GTAmemory::GetVehicleModelName(Hash hash)
{
    ...
    if (modelIt != g_vehicleHashes.end())
...
{
    if (!pedHandle || !GTAmemory::_entityAddressFunc)
        return nullptr;
...
GTAmemory::DrawableCollectionData GTAmemory::BuildDrawableCollectionData(int pedHandle, int componentId)
{
    ...
    for (int g = 0; g < maxGlobal; g++)
    {
        ...
        auto it = std::find_if(raw.begin(), raw.end(), [&](const RawColl& rc) { return rc.name == nameStr; });
        if (it == raw.end())
        {
    ...
    std::sort(raw.begin(), raw.end(), [](const RawColl& a, const RawColl& b) { return a.firstGlobalIdx < b.firstGlobalIdx; });
...
GTAmemory::DrawableCollectionData GTAmemory::BuildPropCollectionData(int pedHandle, int anchorPoint)
{
    ...
    if (!GTAmemory::_entityAddressFunc || !g_GetVariationInfoFromPropIdx || !g_GetDlcPropIdx || !g_GetCollectionName)
        return result;
