...
{
    if (key == VirtualKey::Control && (isWithAlt || get_key_pressed(VirtualKey::Menu) || get_key_pressed(VirtualKey::LeftMenu) || get_key_pressed(VirtualKey::RightMenu)))
        return VirtualKey::RightMenu; // AltGr / right-Alt
    if (key == VirtualKey::Control)
    {
        if (scanCode == 0x1D && isExtended)
