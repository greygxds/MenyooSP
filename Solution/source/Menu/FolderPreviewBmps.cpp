...
decltype(vFolderBmps)::iterator SearchBmps(const std::string& filePath)
{
    ...
    auto last = map.end();
    auto it = std::lower_bound(map.begin(), last, filePath, [](const std::pair<std::string, DxHookIMG::DxTexture>& a, const std::string& b) -> bool { return a.first < b; });
...
void PopulateFolderBmps()
{
    ...
    std::sort(
        vFolderBmps.begin(), vFolderBmps.end(), [](const std::pair<std::string, DxHookIMG::DxTexture>& a, const std::pair<std::string, DxHookIMG::DxTexture>& b) -> bool { return a.first < b.first; }
    );
}
