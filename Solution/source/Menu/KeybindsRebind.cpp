...
{
    if (!ControllerPressed(INPUT_FRONTEND_ACCEPT))
        return false;
...
UINT16 SweepGamepadCandidate()
{
    for (UINT16 c = 0; c <= MaxControlId; ++c)
    {
        ...
        if (c == INPUT_FRONTEND_ACCEPT || c == INPUT_CELLPHONE_SELECT || c == INPUT_FRONTEND_RRIGHT || c == INPUT_FRONTEND_LEFT || (IsPhysicalA(c) && PhysicalAPressed()))
            continue;
...
bool FindConflict(UINT16 value, const KeybindEntry* captureEntry)
{
    ...
    for (const auto& entry : Registry())
    {
        if (&entry == captureEntry)
            continue;
        if (entry.Category() != captureEntry->Category())
            continue; // cross-category duplicates are intentional
        if (keyboard && (entry.KeyboardKey1() == value || entry.KeyboardKey2() == value))
            return true;
        if (!keyboard && (entry.GamepadButton1() == value || entry.GamepadButton2() == value))
            return true;
    }
    return false;
}
...
{
    return g_context == Context::Keyboard ? "~c~ESC / Numpad0 cancel, Backspace unbinds~s~" : "~c~B cancels, D-Pad Left unbinds~s~";
}
...
std::string HeldPromptText()
{
    ...
    return side + " " + ColoredGlyph(g_primary) + " held. ~g~Release to apply~s~, or press another " + SideName() + " for a combo.\n" + CancelHintText();
}

std::string OverwritePromptText()
{
    return ColoredGlyph(g_confirmValue) + " is ~r~already bound~s~.\nPress the same " + SideName() + " again to overwrite, or a different one to cancel.\n" + CancelHintText();
}
...
{
    if (g_lastNotify && GetTickCount() - g_lastNotify < 250)
        return;
    g_lastNotify = GetTickCount();
    Game::Print::ShowNotification(
        "~r~Keybind conflict~s~", ColoredGlyph(conflictValue) + " is already in use.\n~c~Press the same " + SideName() + " again to overwrite,\nor a different one to cancel.~s~", 2.0f
    );
}
...
void Commit(UINT16 first, UINT16 second = NoBind)
{
    ...
    Save();
    addlog(ige::LogType::LOG_DEBUG, "Keybinds: committed \"" + g_entry->Id() + "\" = " + glyph + (combo ? " (combo)" : "") + " (" + ContextName() + ")");
    Game::Print::ShowNotification("~b~" + g_entry->Label() + "~s~", "Bound to ~g~" + glyph + "~s~", 2.0f);
    CancelRebind();
}
...
{
    return vk == VirtualKey::Escape || vk == VirtualKey::Numpad0 || vk == VirtualKey::Return || vk == VirtualKey::Numpad5 || vk == VirtualKey::Back;
}
...
bool RebindTickKeyboard()
{
    ...
    case RebindStage::PrimaryHeld:
    {
        ...
        if (get_key_pressed(g_primary))
        {
            ...
            for (DWORD k = 8; k <= 0xFE; ++k)
            {
                if (k == (DWORD)g_primary || IsReservedKey(k))
                    continue;
...
bool IsRebinding()
{
    return g_stage != RebindStage::Idle;
}

bool IsRebinding(const KeybindEntry* entry, Context context)
{
    if (!IsRebinding() || g_entry != entry)
        return false;
