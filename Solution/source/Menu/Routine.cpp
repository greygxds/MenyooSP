...
void Menu::justopened()
{
    ...
    {
        if (
            IS_DLC_PRESENT(GET_HASH_KEY("mp2023_01_g9ec")) or IS_DLC_PRESENT(GET_HASH_KEY("mp2023_02_g9ec")) or IS_DLC_PRESENT(GET_HASH_KEY("mpchristmas3_g9ec")) or
            IS_DLC_PRESENT(GET_HASH_KEY("mpg9ec")) or IS_DLC_PRESENT(GET_HASH_KEY("mpSum2_G9EC")) or IS_DLC_PRESENT(GET_HASH_KEY("patch2023_01_g9ec")) or
            IS_DLC_PRESENT(GET_HASH_KEY("patchday27g9ecng")) or IS_DLC_PRESENT(GET_HASH_KEY("patchday28g9ecng")) or IS_DLC_PRESENT(GET_HASH_KEY("patchdayg9ecng")) or
            IS_DLC_PRESENT(GET_HASH_KEY("patch2024_01_g9ec")) or IS_DLC_PRESENT(GET_HASH_KEY("mp2024_01_g9ec")) or
        ...
        )
        {
            ...
            ige::myLog << ige::LogType::LOG_WARNING << "Gen9 Content found in dlcpacks, this can cause instability when attempted to be loaded by Menyoo." << std::endl
                       ...
                       << std::endl
                       << "				    Note: mp2024_02 also contains bugged content. If you continue to experience issues, removing this may help" << std::endl
                       << std::endl;
        }
        else if (
...
inline void MenyooMain()
{
    ...
    for (;;)
    {
        ...
        if (loop_neon_rgb || carColorChange)
            TickRainbowFader();
        if (loop_neon_fade == 1)
            TickNeonFadeAnim();
        if (loop_neon_fade == 2)
        ...
        if (loop_neon_fade == 4)
            TickNeonSlideAnim();
        if (loop_neon_flash == 2 || loop_neon_flash == 3)
            TickNeonSpinAnim();
        if (loop_neon_flash == 4)
            TickNeonFwkAnim();
        if (loop_neon_flash == 1)
            TickNeonFlashAnim();
        WAIT(0);
        if (firstTick)
...
void TickSpoonerAutoSave()
{
    ...
    if (sub::Spooner::Databases::EntityDb.empty() && sub::Spooner::Databases::MarkerDb.empty() && sub::Spooner::Databases::LightDb.empty())
        return;
    ...
    GetLocalTime(&t);
    char filename[64];
    snprintf(filename, sizeof(filename), "%04d-%02d-%02dT%02d-%02d-%02d.xml", t.wYear, t.wMonth, t.wDay, t.wHour, t.wMinute, t.wSecond);
    ...
    if (hFind != INVALID_HANDLE_VALUE)
    {
        ...
        {
            std::sort(files.begin(), files.end(), [](auto& a, auto& b) { return CompareFileTime(&a.second, &b.second) < 0; });
...
void TickNeonSlideAnim()
{
    ...
    if (now > g_NeonSliderTick + 20U)
    {
        ...
        int time = now % (2 * loop_neon_delay);
        loop_slide_multiplier = ((abs(1.2 * tanh(1.2 * sin(((0.5 * 3.142 * time / loop_neon_delay))))) - 1) * floor(cos(3.142 * ((time / loop_neon_delay) + 0.5)))) +
                                (1.2 * (-(abs(tanh(1.2 * cos(((0.5 * 3.142 * time) / loop_neon_delay))))) * floor(sin(((3.142 * time) / loop_neon_delay)))));
    ...
    if (GET_GAME_TIMER() > Menu::nextDeferredActionTime)
    {
        for (auto it = fxLoops.begin(); it != fxLoops.end();)
        {
            ...
            case EntityType::PED:
                ...
                {
                    PTFX::TriggerPTFX(
                        it->asset, it->fx, NULL, GET_PED_BONE_COORDS(it->entity.Handle(), Bone::SKEL_Head, 0.0f, 0.0f, 0.0f), it->entity.GetRotation(), GET_RANDOM_FLOAT_IN_RANGE(0.76f, 1.4f)
                    );
                }
                else
                {
...
void SetMassacreModeTick()
{
    ...
    for (GTAentity veh : nearbyVehicles)
    {
        ...
        APPLY_FORCE_TO_ENTITY(
            veh.Handle(), 1, GET_RANDOM_FLOAT_IN_RANGE(1.0f, 9.0f), GET_RANDOM_FLOAT_IN_RANGE(1.0f, 9.0f), GET_RANDOM_FLOAT_IN_RANGE(1.0f, 6.0f), 5.0f, 13.0f, 6.5f, 1, 1, 1, 1, 0, 1
        );
    }
    ...
    for (GTAped ped : nearbyPeds)
    {
        ...
        APPLY_FORCE_TO_ENTITY(
            ped.Handle(), 1, GET_RANDOM_FLOAT_IN_RANGE(1.0f, 9.0f), GET_RANDOM_FLOAT_IN_RANGE(1.0f, 9.0f), GET_RANDOM_FLOAT_IN_RANGE(1.0f, 6.0f), 5.0f, 13.0f, 6.5f, 1, 1, 1, 1, 0, 1
        );
    }
...
void SetPedGun()
{
    if (pedGunRandBit)
    {
        pedGunHash = GET_HASH_KEY(g_pedModels[rand() % (INT)g_pedModels.size()].first);
        pedGunHash.Load(200);
    }
...
void SetTripleBullets()
{
    ...
    GTAentity gunObj = GET_CURRENT_PED_WEAPON_ENTITY_INDEX(playerPed, 0);
    const Vector3& launchPos = gunObj.GetOffsetInWorldCoords(0, gunObj.Dim1().y, 0);
    Vector3 targPos[]{{-2, 2, 2}, {-1.5, 1, 1}, {-1, 0, 0}, {1, 0, 0}, {1.5, 1, 1}, {2, 2, 2}};
...
void SetForgeGun()
{
    ...
    if (g_myWeap == WEAPON_PISTOL && (IS_PLAYER_FREE_AIMING(tempPed) || IS_PLAYER_TARGETTING_ANYTHING(tempPed)))
    {
        ...
        if (forgeDist == 0)
            forgeDist = GameplayCamera::GetPosition().DistanceTo(GET_ENTITY_COORDS(targetSlotEntity, 1));
...
void set_vehicle_neon_anim(GTAvehicle vehicle)
{
    ...
    static constexpr VehicleNeonLight kNeonLights[] = {VehicleNeonLight::Left, VehicleNeonLight::Right, VehicleNeonLight::Front, VehicleNeonLight::Back};
...
{
    const bool anyVehicleWeapon = vehicleRPG || vehicleFireworks || vehicleGuns || vehicleSnowballs || vehicleBalls || vehicleWaterHydrant || vehicleFlameLeak || vehicleLaserGreen ||
                                  vehicleLaserRed || vehicleTurretsValkyrie || vehicleFlaregun || vehicleHeavySniper || vehicleTazerWeapon || vehicleMolotovWeapon || vehicleCombatPDW;
...
static void TickPlayerState(int player, int iped, GTAplayer& player2)
{
    ...
    if (fireworksDisplay)
    {
        StartFireworksAtCoords(
            GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(
                player2.GetPed().Handle(), GET_RANDOM_FLOAT_IN_RANGE(-10.0f, 10.0f), GET_RANDOM_FLOAT_IN_RANGE(-6.0f, 27.0f), GET_RANDOM_FLOAT_IN_RANGE(-9.0f, 3.5f)
            ),
            Vector3(0, 0, GET_RANDOM_FLOAT_IN_RANGE(-90.0f, 90.0f)),
            GET_RANDOM_FLOAT_IN_RANGE(0.4f, 2.45f)
        );
        if (rand() % (INT)2)
        {
            SetExplosionAtCoords(
                iped,
                Vector3(GET_RANDOM_FLOAT_IN_RANGE(9.0f, 25.0f), GET_RANDOM_FLOAT_IN_RANGE(5.0f, 25.0f), GET_RANDOM_FLOAT_IN_RANGE(0.4f, 20.0f)),
                EXPLOSION::DIR_WATER_HYDRANT,
                8.0f,
                0.0f,
                true,
                false,
