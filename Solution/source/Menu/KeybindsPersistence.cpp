...
UINT16 ReadSlotValue(const pugi::xml_node& node, const char* attributeName, const KeybindEntry* entry, UINT16 fallback)
{
    ...
    {
        addlog(ige::LogType::LOG_WARNING, std::string("Keybinds.xml: ") + attributeName + " value " + std::to_string(value) + " out of range for bind \"" + entry->Id() + "\" - attribute ignored");
        return fallback;
    }
    return (UINT16)value;
}
...
bool ReadXml()
{
    ...
    if (result.status != pugi::status_ok)
    {
        if (result.status == pugi::status_file_not_found)
            ...
            addlog(ige::LogType::LOG_ERROR, std::string("Keybinds.xml: failed to load (") + result.description() + ") - binds fell back to defaults/legacy ini import");
        return false;
    }
    ...
    for (pugi::xml_node node : root.children("Bind"))
    {
        ...
        if (entry == nullptr)
        {
            addlog(ige::LogType::LOG_WARNING, std::string("Keybinds.xml: unknown bind id \"") + (id[0] != '\0' ? id : "<missing>") + "\" - entry skipped");
            continue;
        }
...
void ImportFromIni()
{
    ...
    for (const auto& item : legacy)
    {
        ...
        if (entry == nullptr)
        {
            addlog(ige::LogType::LOG_WARNING, std::string("Legacy ini import: [") + item.section + "] " + item.key + " references unknown bind id \"" + item.id + "\" - skipped");
            continue;
        }
        LONG value = ini.GetLongValue(item.section, item.key, -1);
        if (value < 0)
            continue; // unset key: keep the registry default
        if (value > (LONG)NoBind)
        {
            addlog(
                ige::LogType::LOG_WARNING,
                std::string("Legacy ini import: [") + item.section + "] " + item.key + " value " + std::to_string(value) + " out of range for bind \"" + item.id + "\" - kept default"
            );
            continue;
        }
        ...
        {
        case 0:
            entry->SetKeyboardKey1((UINT16)value);
            break;
        case 1:
            entry->SetGamepadButton1((UINT16)value);
            break;
        case 2:
            entry->SetGamepadButton2((UINT16)value);
            break;
        }
    }
    WriteXml();
