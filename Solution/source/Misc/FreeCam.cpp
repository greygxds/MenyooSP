...
void SyncControlledEntity()
{
    ...
    const int vehicleHandle = IS_PED_IN_ANY_VEHICLE(playerPed.GetHandle(), false) ? GET_VEHICLE_PED_IS_IN(playerPed.GetHandle(), false) : 0;
...
Input ReadKeyboardInput()
{
    ...
    input.translation.x = GET_DISABLED_CONTROL_NORMAL(0, INPUT_MOVE_LR) * movement;
    input.translation.y = -GET_DISABLED_CONTROL_NORMAL(0, INPUT_MOVE_UD) * movement;
    input.translation.z = IS_DISABLED_CONTROL_PRESSED(2, INPUT_PARACHUTE_BRAKE_RIGHT) ? movement : IS_DISABLED_CONTROL_PRESSED(2, INPUT_PARACHUTE_BRAKE_LEFT) ? -movement : 0.0f;
    return input;
}
...
void ResetControllerPedStance()
{
    ...
    if (GetControlledEntity() != state.ped.entity || !ped.Exists())
        return;
...
void Start()
{
    if (state.active || sub::Spooner::SpoonerMode::bEnabled)
        return;
