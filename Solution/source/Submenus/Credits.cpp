...
static std::string FetchSupportersJson()
{
    ...
    HINTERNET hSession = WinHttpOpen(L"Menyoo/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (!hSession)
        return result;
    ...
    HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"GET", kApiPath, nullptr, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE);
    if (!hRequest)
    {
...
void CreditsMenu()
{
    ...
    if (openPatreon)
    {
        ShellExecuteA(nullptr, "open", "https://www.patreon.com/cw/ItsJustCurtis", nullptr, nullptr, SW_SHOWNORMAL);
    }
