...
UINT GetNumberOfEntitiesSpawned(const EntityType& type)
{
    ...
    case EntityType::ALL:
        ...
        break;
    default:
        return (UINT)std::count_if(Databases::EntityDb.begin(), Databases::EntityDb.end(), [type](const SpoonerEntity& item) { return item.type == type; });
        break;
    }
}
...
SpoonerEntity AddPed(const GTAmodel::Model& model, const std::string& name, bool unloadModel)
{
    ...
    if (!spoocam.IsActive())
    {
        ...
        newEntity.handle = World::CreatePed(
            model, myPedOrVehicle.GetOffsetInWorldCoords(0, myPedOrVehicle.Dim1().y + 2.6f + dimensions.Dim2.y, 0), myPedOrVehicle.GetRotation(), myPedOrVehicle.HeightAboveGround() < 3.0f
        );
        if (unloadModel)
            model.Unload();
    }
...
SpoonerEntity AddEntityOfType(const EntityType& type, const GTAmodel::Model& model, const std::string& name)
{
    ...
    case EntityType::PROP:
        return AddProp(model, name);
        break;
    case EntityType::PED:
        return AddPed(model, name);
        break;
    case EntityType::VEHICLE:
        return AddVehicle(model, name);
        break;
    }
    return SpoonerEntity();
}
...
SpoonerEntity CopyEntity(SpoonerEntity orig, bool isInDb, bool addToDb, UINT8 copyAttachments, bool unloadModel, UINT8 currAtir)
{
    ...
    {
        if (Databases::BlipDb[i].BlipType == SpoonerBlip::Type::Entity && Databases::BlipDb[i].EntityHandle == orig.handle.GetHandle())
        {
            SpoonerBlip* newBlip = sub::Spooner::BlipCustoms::AddBlip(SpoonerBlip::Type::Entity, Databases::BlipDb[i].Name);
