...
{
    if (Menu::activeSubmenu == (int)SUB::SPOONER_JOBBROWSER)
        return 0; // Bookmarked
    if (Menu::activeSubmenu == (int)SUB::SPOONER_JOBBROWSER_MYCONTENT)
        return 1; // My Content
    if (Menu::activeSubmenu == (int)SUB::SPOONER_JOBBROWSER_FRIENDCONTENT)
        return 2; // Friend Content
    if (Menu::activeSubmenu == (int)SUB::SPOONER_JOBBROWSER_MOSTRECENT)
        return 3; // Most Recent images don't seem to load with this one but maybe cause too many
    if (Menu::activeSubmenu == (int)SUB::SPOONER_JOBBROWSER_TOPRATED)
        return 4; // Top Rated
    if (Menu::activeSubmenu == (int)SUB::SPOONER_JOBBROWSER_CREWCONTENT)
        return 5; // Crew Content
    return 0;
}
...
static std::string SanitizeFilename(std::string name)
{
    ...
    name.erase(std::remove_if(name.begin(), name.end(), [&](char c) { return invalidChars.find(c) != std::string::npos; }), name.end());
...
static void ImportThreadProc(std::string url, bool saveImg, std::vector<std::string> langs, int resolvedFileVersion, int resolvedCategory)
{
    ...
    bool isDirectJson = (url.find("prod.cloud.rockstargames.com") != std::string::npos) && (url.length() > 5 && url.compare(url.length() - 5, 5, ".json") == 0);
    ...
    if (isDirectJson)
    {
        ...
        if (success && !body.empty())
        {
            try
            {
                ...
                std::string raceName = "";
                if (jsonData.contains("mission") && jsonData["mission"].is_object() && jsonData["mission"].contains("gen") && jsonData["mission"]["gen"].is_object() &&
...
static std::vector<JobEntity> ExtractEntitiesFromSection(const json& section, const std::string& entityType, const std::string& tintFieldName)
{
    ...
    auto getUInt = [](const json& j, unsigned int def = 0) -> unsigned int
    {
        ...
        if (j.is_string())
        {
            try
            {
                return std::stoul(j.get<std::string>());
            }
            catch (...)
            {
            }
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (section.contains("loc") && section["loc"].is_array() && i < section["loc"].size())
        {
            ...
            if (locVal.is_object())
            {
                if (locVal.contains("x"))
                    pos.x = getFloat(locVal["x"]);
                if (locVal.contains("y"))
                    pos.y = getFloat(locVal["y"]);
                if (locVal.contains("z"))
                    pos.z = getFloat(locVal["z"]);
            }
        }
        ...
        if (section.contains("vRot") && section["vRot"].is_array() && i < section["vRot"].size())
        {
            ...
            if (rotVal.is_object())
            {
                if (rotVal.contains("x"))
                    rot.x = getFloat(rotVal["x"]);
                if (rotVal.contains("y"))
                    rot.y = getFloat(rotVal["y"]);
                if (rotVal.contains("z"))
                    rot.z = getFloat(rotVal["z"]);
                hasRot = true;
            }
        }
...
bool ConvertJsonToXml(const std::string& jsonStr, const std::string& xmlOutPath, const std::string& defaultName, std::string& outRaceName)
{
    ...
    std::string raceName = "";
    if (mission.contains("gen") && mission["gen"].is_object() && mission["gen"].contains("nm") && mission["gen"]["nm"].is_string())
    {
    ...
    if (mission.contains("fsp") && mission["fsp"].is_object() && mission["fsp"].contains("loc") && mission["fsp"]["loc"].is_array() && mission["fsp"]["loc"].size() > 0)
    {
