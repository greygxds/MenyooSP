...
void DrawAll()
{
    for (auto& blip : Databases::BlipDb)
    {
        ...
        World::DrawLightWithRange(pos, bSelectedInSub ? RGBA(baseColour.Inverse(false), 150) : baseColour, 2.3f, 1.5f);
    }
}
...
void UpdateAttachedBlips()
{
    for (auto& blip : Databases::BlipDb)
    {
        if (blip.bAttached && blip.EntityHandle != 0 && ENTITY::DOES_ENTITY_EXIST(blip.EntityHandle) && blip.BlipType != SpoonerBlip::Type::Entity)
        {
