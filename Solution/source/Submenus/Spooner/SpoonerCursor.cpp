...
bool ShouldDraw()
{
    return SpoonerMode::bEnabled && !WardrobeCamera::IsBusy() && SpoonerMode::editingState.mode == SpoonerMode::eEditMode::Disabled && SpoonerCamera::camera.Exists();
}
...
{
    return ShouldDraw() ? SpoonerCamera::camera.RaycastForEntity(Vector2(0.0f, 0.0f), 0, 160.0f) : GTAentity();
}
...
{
    if (state.dragging || !entity.Exists())
        return;
...
void UpdateDrag()
{
    ...
    const Vector3 position = SpoonerCamera::camera.RaycastForCoord(Vector2(0.0f, 0.0f), entity, 90.0f, 15.0f + dimensions.Dim2.y);
...
{
    const SpoonerEntity copy = EntityManagement::CopyEntity(SpoonerMode::GetEntityPtrValue(target), isInDb, true, Submenus::_copyEntTexterValue);
    if (!copy.handle.Exists())
        return;
...
{
    if (Menu::activeSubmenu != SUB::CLOSED || !target.Exists())
        return;
...
{
    const float deltaSeconds = state.lastUpdateTime == 0 ? 0.0f : static_cast<float>(now - state.lastUpdateTime) / 1000.0f;
...
void Draw()
{
    if (state.visibilityOpacity <= 0.0f)
        return;
