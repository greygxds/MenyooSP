...
struct PendingWrites
{
    ...
    Vector3 scaleVal{1.0f, 1.0f, 1.0f};
};
...
static void BuildCameraMatricesFromCache(const Vector3& camCoord, const Vector3& camRot, float camFov, float screenW, float screenH, float* outView, float* outProj)
{
    constexpr float DEG2RAD = 3.14159265358979323846f / 180.0f;
...
static void Mat4Mul(const float a[16], const float b[16], float out[16])
{
    for (int row = 0; row < 4; row++)
        ...
        {
            out[row + col * 4] = a[row + 0 * 4] * b[0 + col * 4] + a[row + 1 * 4] * b[1 + col * 4] + a[row + 2 * 4] * b[2 + col * 4] + a[row + 3 * 4] * b[3 + col * 4];
        }
}
...
static void RunGizmo_NoLock(SharedState& s)
{
    ...
    if (!s.cache.entityValid || s.render.editingState.mode != SpoonerMode::eEditMode::Gizmo)
        return;
    ...
    if (op == ImGuizmo::TRANSLATE)
    {
        ...
        ImGuizmo::Manipulate(viewMat, projMat, op, gizmoMode, matrix, deltaMatrix, (s.render.gridSnapEnabled && s.render.gridSnapSize > 0.0f) ? snapMatrix : nullptr);

        if (ImGuizmo::IsUsing())
        {
            Vector3 newPos(s.cache.position.x + deltaMatrix[12], s.cache.position.y + deltaMatrix[13], s.cache.position.z + deltaMatrix[14]);

            if (fabsf(newPos.x - s.cache.position.x) > FLT_EPSILON || fabsf(newPos.y - s.cache.position.y) > FLT_EPSILON || fabsf(newPos.z - s.cache.position.z) > FLT_EPSILON)
            {
    ...
    else if (op == ImGuizmo::ROTATE)
    {
        ...
        ImGuizmo::Manipulate(viewMat, projMat, op, gizmoMode, s_DragMatrix, nullptr, (s.render.gridSnapEnabled && s.render.rotationSnapDegrees > 0.0f) ? snapMatrix : nullptr);
        ...
        if (ImGuizmo::IsUsing())
        {
            ...
            if (fabsf(newRot.x - oldRot[0]) > FLT_EPSILON || fabsf(newRot.y - oldRot[1]) > FLT_EPSILON || fabsf(newRot.z - oldRot[2]) > FLT_EPSILON)
            {
                s.pending.entityHandle = s.cache.entityHandle;
    ...
    else if (op == ImGuizmo::SCALE)
    {
        ...
        if (ImGuizmo::IsUsing())
        {
            ...
            if (fabsf(newScale.x - s.cache.scale.x) > FLT_EPSILON || fabsf(newScale.y - s.cache.scale.y) > FLT_EPSILON || fabsf(newScale.z - s.cache.scale.z) > FLT_EPSILON)
            {
                s.pending.entityHandle = s.cache.entityHandle;
...
static void ApplyPending_ScriptThread(PendingWrites pending)
{
    ...
    if (sel.handle.Exists())
    {
        ...
        {
            if (pending.positionDirty)
                sel.handle.SetPosition(SpoonerMode::SnapPos(pending.positionVal));
            if (pending.rotationDirty)
                sel.handle.SetRotation(SpoonerMode::SnapRot(pending.rotationVal));
        }
        // Attached entity - converting to local offsets
        else if (parentEntity.Exists())
        {
            if (pending.positionDirty)
                GetAttachmentOffset(sel, parentEntity, pending.positionVal);
            if (pending.rotationDirty)
            {
        ...
        if (pending.scaleDirty)
        {
            ...
            Submenus::EntityScaleState& state = [&]() -> Submenus::EntityScaleState&
            {
                ...
                case EntityType::VEHICLE:
                    return Submenus::_vehScale;
                case EntityType::PED:
                    return Submenus::_pedScale;
                default:
                    return Submenus::_objScale;
                }
            }();
