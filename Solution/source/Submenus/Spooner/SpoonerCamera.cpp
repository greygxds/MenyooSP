...
static void EnsureActive(const GTAped& playerPed)
{
    if (!camera.Exists())
    {
        ...
        camera = World::CreateCamera(playerPosition + Vector3(0, 0, 2.8f), Vector3(0, 0, playerPed.GetRotation().z), MenuConfig::FreeCam::defaultFov);
        camera.SetActive(false);
    }
...
static Input ReadKeyboardInput()
{
    ...
    if (!editingState.BlocksCameraTranslation())
    {
        ...
        float movementSensitivity = IS_DISABLED_CONTROL_PRESSED(0, INPUT_DUCK) ? MenuConfig::FreeCam::defaultSlowSpeed : MenuConfig::FreeCam::defaultSpeed;
...
static void AdjustSpeed(int scroll)
{
    ...
    const float newSpeed = std::clamp(currentSpeed + scroll * MenuConfig::FreeCam::speedAdjustStep, MenuConfig::FreeCam::minSpeed, MenuConfig::FreeCam::maxSpeed);
...
static void AdjustFov(int scroll)
{
    ...
    const float newFov = std::clamp(currentFov + scroll * MenuConfig::FreeCam::fovAdjustStep, MenuConfig::FreeCam::minFov, MenuConfig::FreeCam::maxFov);
...
{
    if (!playerPed.Exists() || !camera.Exists())
        return;
    if (playerPed.GetPosition().DistanceTo(camera.GetPosition()) <= 350.0f)
        return;
...
void Tick()
{
    ...
    const Input input = Menu::usingControllerInput ? ReadControllerInput() : ReadKeyboardInput();
