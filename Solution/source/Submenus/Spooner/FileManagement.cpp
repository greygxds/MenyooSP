...
const std::vector<std::string> drawableComponentSlotNames = {"head", "berd", "hair", "uppr", "lowr", "hand", "feet", "teef", "accs", "task", "decl", "jbib"};
const std::vector<std::string> drawablePropSlotNames = {"p_head", "p_eyes", "p_ears", "p_mouth", "p_lhand", "p_rhand", "p_lwrist", "p_rwrist", "p_hip", "p_lfoot", "p_rfoot", "p_unk1", "p_unk2"};
const std::vector<std::string> facialFeatureSlotNames = {"NoseWidth",       "NoseHeight",     "NoseLength", "NoseBridge", "NoseTip", "NoseBridgeShaft", "BrowHeight", "BrowWidth",
                                                         "CheekboneHeight", "CheekboneWidth", "CheekWidth", "Eyelids",    "Lips",    "JawWidth",        "JawHeight",  "ChinLength",
                                                         "ChinPosition",    "ChinWidth",      "ChinShape",  "NeckWidth"};
const std::vector<std::string> overlaySlotNames = {"SkinRash", "Beard", "Eyebrows", "Wrinkles", "Makeup", "Blush", "Pigment1", "Pigment2", "Lipstick", "Spots", "ChestHair", "Chest1", "Chest2"};
...
void LoadPedHeadFeaturesFromXml(GTAped ep, const pugi::xml_node& nodePedHeadFeatures, const GTAmodel::Model& eModel)
{
    ...
    if (!g_unlockMaxIDs && (headBlend.shapeFirstID > 45 || headBlend.shapeSecondID > 45 || headBlend.shapeThirdID > 45))
    {
        ...
        addlog(
            ige::LogType::LOG_WARNING,
            "Ped Head Index " + std::to_string(max(headBlend.shapeFirstID, max(headBlend.shapeSecondID, headBlend.shapeThirdID))) +
                " outside normal range of 0-45. Ensure Matching Addon Heads are installed from XML Source and Max Head IDs are unlocked."
        );
    }
    ...
    for (auto node = nodePedHeadOverlays.first_child(); node; node = node.next_sibling(), overlayIndex++)
    {
        ...
        SET_PED_HEAD_OVERLAY_TINT(
            ep.Handle(),
            overlayIndex,
            sub::PedHeadFeatures_catind::GetPedHeadOverlayColourType((PedHeadOverlay)overlayIndex),
            pedHead.overlayData[overlayIndex].colour,
            pedHead.overlayData[overlayIndex].colourSecondary
        );
    }
...
void LoadPedDecalsFromXml(GTAped ep, const pugi::xml_node& nodePedDecals)
{
    ...
    {
        sub::PedDecals::PedDecalValue decal(node.attribute("collection").as_uint(), node.attribute("value").as_uint());
...
SpoonerEntityWithInitHandle SpawnEntityFromXmlNode(pugi::xml_node& nodeEntity, std::unordered_set<Hash>& vModelHashes)
{
    ...
    if (opacityLevel < 255)
        e.e.handle.SetAlpha(opacityLevel);
    ...
    if (nodeEntity.child("MaxHealth"))
        e.e.handle.SetMaxHealth(nodeEntity.child("MaxHealth").text().as_int());
    if (nodeEntity.child("Health"))
        e.e.handle.SetHealth(nodeEntity.child("Health").text().as_int());
...
bool SaveDbToFile(const std::string& filePath, bool bForceReferenceCoords)
{
    ...
    if (bForceReferenceCoords)
    {
        ...
        for (;;)
        {
            ...
            Game::CustomHelpText::ShowThisFrame(
                oss_ << "Press ~INPUT_LOOK_BEHIND~ to save " << (bSpoocamIsActive ? "the camera target position" : "your current position") << " as the map reference."
            );
...
bool LoadPlacementsFromFile(const std::string& filePath)
{
    ...
    if (nodeReferenceCoords && Settings::bTeleportToReferenceWhenLoadingFile)
        TeleportNetPed(myPed.Handle(), refCoords.x, refCoords.y, refCoords.z);
    else if (nodeImgLoadingCoords)
        TeleportNetPed(myPed.Handle(), myPos.x, myPos.y, myPos.z);
    WAIT(200);
...
bool LoadPlacementsFromSP00NFile(const std::string& filePath)
{
    ...
    for (auto& section : vSections)
    {
        ...
        e.hashName = e.type == EntityType::PROP ? get_prop_model_label(eModel) : (e.type == EntityType::PED ? GetPedModelLabel(eModel, true) : get_vehicle_model_label(eModel, true));
        if (e.hashName.length() == 0)
            e.hashName = IntToHexString(eModel.hash, true);
        e.isStill = true;
        ...
        if (opacityLevel < 255)
            e.handle.SetAlpha(opacityLevel);
