...
void UpdatePreviewRotation()
{
    if (modelPreviewInfo.entity.Exists() && Menu::activeSubmenu != SUB::CLOSED)
    {
        ...
        bool dpadPressed = IS_DISABLED_CONTROL_PRESSED(2, INPUT_FRONTEND_LEFT) || IS_DISABLED_CONTROL_PRESSED(2, INPUT_FRONTEND_RIGHT) || IS_DISABLED_CONTROL_PRESSED(2, INPUT_FRONTEND_UP) ||
                           IS_DISABLED_CONTROL_PRESSED(2, INPUT_FRONTEND_DOWN);

        if (!dpadPressed)
        {
            if (lbPressed && !rbPressed)
                previewYawOffset -= 2.0f;
            if (rbPressed && !lbPressed)
                previewYawOffset += 2.0f;
            if (previewYawOffset > 360.0f || previewYawOffset < -360.0f)
                previewYawOffset = fmod(previewYawOffset, 360.0f);
        }
    }
}
...
bool GetEntityPtr(GTAentity& inEntity, SpoonerEntity*& outEntity)
{
    ...
    outEntity->hashName = outEntity->type == EntityType::PROP ? get_prop_model_label(outEntityModel)
                                                              : (outEntity->type == EntityType::PED ? GetPedModelLabel(outEntityModel, true) : get_vehicle_model_label(outEntityModel, true));
    if (outEntity->hashName.length() == 0)
        outEntity->hashName = IntToHexString(outEntityModel.hash, true);
...
{
    if (!bEnabled || !Settings::bShowModelPreviews || IS_PAUSE_MENU_ACTIVE())
        return;
    if (SpoonerCursor::IsDragging())
        return;
...
static void DrawSpoonerWorldItems()
{
    ...
    if (!Databases::BlipDb.empty())
    {
        ...
        bool bInBlipSub = sub == SUB::SPOONER_BLIPS || sub == SUB::SPOONER_BLIPS_ADD_SELECT || sub == SUB::SPOONER_BLIPS_RADIALINBLIP || sub == SUB::SPOONER_BLIPS_COORDINBLIP ||
                          sub == SUB::SPOONER_BLIPS_ENTITYINBLIP || sub == SUB::SPOONER_BLIPS_ATTACH || sub == SUB::SPOONER_BLIPS_ENTITY_SELECT || sub == SUB::SPOONER_BLIPS_ICONS;
...
void ProcessKeyboardManipulation(Vector3& position, Vector3& rotation)
{
    ...
    {
        float snapAmount = editingState.transformMode == eTransformMode::Rotation ? Settings::rotationSnapDegrees : Settings::gridSnapSize;
        if (snapAmount > 0.0f)
            step = snapAmount;
    }

    auto& target = editingState.transformMode == eTransformMode::Rotation ? rotation : position;
    if (IsKeyDown(VirtualKey::W))
        target.x += step;
    if (IsKeyDown(VirtualKey::S))
        target.x -= step;
    if (IsKeyDown(VirtualKey::A))
        target.y += step;
    if (IsKeyDown(VirtualKey::D))
        target.y -= step;
    if (IsKeyDown(VirtualKey::E))
        target.z += step;
    if (IsKeyDown(VirtualKey::Q))
        target.z -= step;
...
void DrawEditingHUD()
{
    ...
    auto glyph = [](const std::string& id) { return Keybinds::GetGlyph(id); };

    if (!bEnabled)
    {
        drawText("~r~Entity manipulation requires the Spooner Camera.");
        drawText("~b~Press " + glyph("spooner_mode") + ":~w~ Enable Spooner Mode.");
        return;
    }
    ...
    if (editingState.mode == eEditMode::Disabled)
    {
        ...
        drawText("~b~Press " + glyph("spooner_edit_mode") + ":~w~ Enable keyboard controls or gizmo editing mode.");
    }
    else if (editingState.mode == eEditMode::Keyboard)
    {
        if (editingState.transformMode == eTransformMode::Rotation)
        {
            ...
            drawText("~b~A/D: ~w~Roll+ / Roll-");
            drawText("~b~E/Q: ~w~Yaw+ / Yaw-");
            drawText("~b~" + glyph("spooner_sensitivity_down") + "/" + glyph("spooner_sensitivity_up") + ": ~w~+/- Sensitivity");
            drawText("~b~" + glyph("spooner_edit_transform") + ": ~w~Edit position");
        }
        else
        {
            ...
            drawText("~b~" + glyph("spooner_sensitivity_down") + "/" + glyph("spooner_sensitivity_up") + ": ~w~+/- Sensitivity");
            drawText("~b~" + glyph("spooner_edit_transform") + ": ~w~Edit rotation");
        }
        drawText("~b~" + glyph("spooner_edit_copy") + ": ~w~Copy entity");
        drawText("~b~" + glyph("spooner_edit_mode") + ": ~w~Switch to gizmo / disable controls.");
    }
    else if (editingState.mode == eEditMode::Gizmo)
    {
        ...
        drawText("~b~" + glyph("spooner_edit_transform") + ":~w~ Cycle mode");
        drawText("~b~" + glyph("spooner_camera_lock") + ":~w~ " + (editingState.cameraLocked ? "Unlock camera" : "Lock camera"));
        drawText("~b~" + glyph("spooner_local_space") + ":~w~ " + (editingState.localSpace ? "Edit in world space" : "Edit in local space"));
        drawText("~b~" + glyph("spooner_edit_copy") + ":~w~ Copy entity");
        drawText("~b~" + glyph("spooner_edit_mode") + ":~w~ Disable gizmo mode");
    }
}
