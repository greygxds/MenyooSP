...
void GoToCoord::RunP(GTAped& ep)
{
    TASK_GO_STRAIGHT_TO_COORD(
        ...
        Vector3::DirectionToRotation(Vector3::Normalize(this->destination - ep.GetPosition())).z,
        0.0f
    );
}
...
void FollowEntity::RunP(GTAped& ep)
{
    TASK_FOLLOW_TO_OFFSET_OF_ENTITY(
        ...
        GET_RANDOM_FLOAT_IN_RANGE(-1.5f, 0.5f),
        0.0f,
        this->speed,
        this->durationAfterLife > 0 ? -1 : this->duration,
        0.1f,
...
void PlayAnimation::Run(void* ve)
{
    ...
    {
        PLAY_ENTITY_ANIM(
            entity.Handle(), this->animName.c_str(), this->animDict.c_str(), 8.0f, static_cast<BOOL>(this->flag & AnimFlag::Loop), static_cast<BOOL>(this->flag & AnimFlag::StayInLastFrame), 0, 0.0f, 0
        );
    }
}
...
{
    ep.Task().EnterVehicle(
        this->vehicleToEnter, this->seatIndex == -2 ? this->vehicleToEnter.FirstFreeSeat(SEAT_DRIVER) : (VehicleSeat)this->seatIndex, this->durationAfterLife > 0 ? -1 : this->duration + 200
    );
}
