...
class SpoonerBlipPosition
{
  public:
    struct Attachment_t
    {
        ...
        Attachment_t() : attachedTo(0), offset(Vector3(0, 0, 0))
        {
        }
    ...
    SpoonerBlipPosition() : m_position(Vector3(0, 0, 0))
    {
    }
...
class SpoonerBlip
{
  public:
    ...
    SpoonerBlip(const std::string& name, const Vector3& position, const Vector3& rotation) : Name(name), X(position.x), Y(position.y), Z(position.z), Rotation(rotation)
    {
    }
