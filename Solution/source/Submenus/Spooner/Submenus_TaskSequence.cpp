...
void FaceDirection()
{
    ...
    World::DrawMarker(
        MarkerType::DebugSphere, selectedEntity.handle.GetPosition().PointOnCircle(1.0f, tskPtr->heading), Vector3(0, 0, tskPtr->heading), Vector3(), Vector3(0.3f, 0.3f, 0.3f), RGBA(177, 33, 193, 210)
    );
...
void SlideToCoord()
{
    ...
    World::DrawMarker(
        MarkerType::DebugSphere, selectedEntity.handle.GetPosition().PointOnCircle(1.0f, tskPtr->heading), Vector3(), Vector3(0, 0, tskPtr->heading), Vector3(0.3f, 0.3f, 0.3f), RGBA(177, 33, 193, 210)
    );
...
void PlaySpeechWithVoice_inVoice()
{
    ...
    auto paramIter =
        std::find_if(sub::Speech::vSpeechParams.begin(), sub::Speech::vSpeechParams.end(), [tskPtr](const sub::Speech::SpeechParamS& item) { return item.label.compare(tskPtr->paramName) == 0; });
...
void DriveLandPlane()
{
    ...
    for (auto& nas : std::vector<std::pair<std::string, Vector3*>>{{"Runway Start", &tskPtr->runwayStart}, {"Runway End", &tskPtr->runwayEnd}})
    {
        AddBreak(nas.first);
        ...
        bool bManualEditingForPosPressed = false;
        AddOption("Adjust Manually", bManualEditingForPosPressed, nullFunc, SUB::SPOONER_VECTOR3_MANUALEDITING);
        if (bManualEditingForPosPressed)
        {
...
void AchievePushForce()
{
    ...
    bool offsetx_plus = 0, offsetx_minus = 0, offsety_plus = 0, offsety_minus = 0, offsetz_plus = 0, offsetz_minus = 0, bResetOffsetVector = 0;
    ...
    if (offsetx_plus)
        tskPtr->offsetVector.x += SpoonerMode::editingState.precisionPos;
    if (offsetx_minus)
        tskPtr->offsetVector.x -= SpoonerMode::editingState.precisionPos;
    if (offsety_plus)
        tskPtr->offsetVector.y += SpoonerMode::editingState.precisionPos;
    if (offsety_minus)
        tskPtr->offsetVector.y -= SpoonerMode::editingState.precisionPos;
    if (offsetz_plus)
        tskPtr->offsetVector.z += SpoonerMode::editingState.precisionPos;
    if (offsetz_minus)
        tskPtr->offsetVector.z -= SpoonerMode::editingState.precisionPos;
}
...
void SetRotation()
{
    ...
    bool rotx_plus = 0, rotx_minus = 0, roty_plus = 0, roty_minus = 0, rotz_plus = 0, rotz_minus = 0, bResetRotVector = 0;
    ...
    if (rotx_plus && tskPtr->rotationValue.x < 180.0f)
        tskPtr->rotationValue.x += SpoonerMode::editingState.precisionRot;
    if (rotx_minus && tskPtr->rotationValue.x > -180.0f)
        tskPtr->rotationValue.x -= SpoonerMode::editingState.precisionRot;
    if (roty_plus && tskPtr->rotationValue.y < 180.0f)
        tskPtr->rotationValue.y += SpoonerMode::editingState.precisionRot;
    if (roty_minus && tskPtr->rotationValue.y > -180.0f)
        tskPtr->rotationValue.y -= SpoonerMode::editingState.precisionRot;
    if (rotz_plus && tskPtr->rotationValue.z < 180.0f)
        tskPtr->rotationValue.z += SpoonerMode::editingState.precisionRot;
...
void Sub_TaskSequence_TaskList()
{
    ...
    for (UINT16 i = 0; i < taskSequence.TaskCount(); i++)
    {
        ...
        if (Menu::IsLastDrawnOptionSelected())
        {
            ...
            bMoveTaskPressed = MenuPressTimer::IsButtonHeldOrTapped(MenuPressTimer::Button::Left) ? -1 : MenuPressTimer::IsButtonHeldOrTapped(MenuPressTimer::Button::Right) ? 1 : 0;
    ...
    if (thisDuration >= 0) // -1 for tasks with no settings. -2 for tasks with settings but no duration setting.
    {
        ...
        if (bDuration_plus)
        {
            addlog(
                ...
                "Increasing duration by " + std::to_string(SpoonerMode::editingState.precisionPos * 1000) + " milliseconds. Target " +
                    std::to_string(thisDuration + SpoonerMode::editingState.precisionPos * 1000)
            );
            if (thisDuration <= INT_MAX - SpoonerMode::editingState.precisionPos * 1000)
                thisDuration += static_cast<int>(SpoonerMode::editingState.precisionPos * 1000);
        ...
        if (bDuration_minus)
        {
            addlog(
                ige::LogType::LOG_TRACE,
                "Decreasinc duration by " + std::to_string(SpoonerMode::editingState.precisionPos * 1000) + " milliseconds. Target " +
                    std::to_string(thisDuration - SpoonerMode::editingState.precisionPos * 1000)
            );
            if (thisDuration > SpoonerMode::editingState.precisionPos * 1000)
                thisDuration -= static_cast<int>(SpoonerMode::editingState.precisionPos * 1000);
            addlog(ige::LogType::LOG_TRACE, "New duration is " + std::to_string(thisDuration) + " milliseconds.");
        }
        if (bDuration_input)
