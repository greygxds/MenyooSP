...
class GTAMapMod
{
  ...
  public:
    GTAMapMod(const std::string& newName, const std::string& newAuthor, Vector3 newLocation, pugi::xml_node node) : name(newName), author(newAuthor), location(newLocation), xmlNode(node)
    {
    }
...
{
    bool pressed = false;
    AddOption(mapMod.Name(), pressed, nullFunc, SUB::MAPMODS2, false);
    if (pressed)
    {
