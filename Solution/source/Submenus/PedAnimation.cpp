...
bool RebuildFavCache(const std::string& searchStr)
{
    ...
    std::sort(
        s_favCache.sortedCategoryNames.begin(),
        s_favCache.sortedCategoryNames.end(),
...
void StopAnimation(GTAentity entity, bool clearScenario = false)
{
    ...
    if (ctx.dbIndex >= 0)
    {
        ...
        if (clearScenario)
            spoonerEntity.currentScenario.clear();

        if (ctx.attachedTo.Exists() && spoonerEntity.attachmentArgs.isAttached)
        {
            spoonerEntity.handle.AttachTo(
                ctx.attachedTo, spoonerEntity.attachmentArgs.boneIndex, spoonerEntity.handle.GetIsCollisionEnabled(), spoonerEntity.attachmentArgs.offset, spoonerEntity.attachmentArgs.rotation
            );
        }
        ...
        if (sub::Spooner::selectedEntity.handle.Equals(spoonerEntity.handle))
        {
            ...
            if (clearScenario)
                sub::Spooner::selectedEntity.currentScenario = spoonerEntity.currentScenario;
...
void PlayAnimation(GTAentity entity, const std::string& animDict, const std::string& animName)
{
    ...
    if (entity.IsPed())
    {
        TASK_PLAY_ANIM(
            ...
            animDict.c_str(),
            animName.c_str(),
            g_customAnimSettings.speed,
            g_customAnimSettings.speedMult,
            g_customAnimSettings.duration,
            g_customAnimSettings.flag,
            g_customAnimSettings.playbackRate,
            g_customAnimSettings.lockPos,
            g_customAnimSettings.lockPos,
            g_customAnimSettings.lockPos
        );
    }
    ...
    {
        PLAY_ENTITY_ANIM(entity.Handle(), animName.c_str(), animDict.c_str(), 8.0f, static_cast<BOOL>(g_customAnimSettings.flag & 1), static_cast<BOOL>(g_customAnimSettings.flag & 2), 0, 0.0f, 0);
    }
...
void AllPedAnimsMenu()
{
    ...
    for (auto& current : allPedAnims)
    {
        if (!current.second.empty())
        {
            ...
            AddOption(current.first, currentPressed);
            if (currentPressed)
            {
...
std::vector<std::string> vValues_TaskScenarios{
    "bbq_registered",
...
void AddMoveGroupOption(const std::string& text, std::string moveGroup = "", bool& extraOptionCode = null)
{
    ...
    bool pressed = false;
    AddTickol(text, moveGroupIsActive, pressed, pressed);
    if (pressed)
    {
