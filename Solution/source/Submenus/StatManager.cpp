...
const std::array<NamedCharStatList_t, 4> vCharStatLists //Change 4 to 5 when properties are added back in
    {{
        {"Cash", {{"TOTAL_CASH", "Total Cash", StatDataType_t::INT, 0, static_cast<float>(INT_MAX)}}},
        {"Abilities (ALPHA)",
        ...
        {"Special Ability", {{"SPECIAL_ABILITY", "Amount Not Unlocked (ALPHA)", StatDataType_t::INT, 0, 100}, {"SPECIAL_ABILITY_UNLOCKED", "Special Capacity", StatDataType_t::INT, 0, 100}}},
        {"K/D Ratio", {{"KILLS", "Kill Count", StatDataType_t::INT, 0, static_cast<float>(INT_MAX)}, {"DEATHS", "Death Count", StatDataType_t::INT, 0, static_cast<float>(INT_MAX)}}} /*,
			{"Properties",{
...
const std::array<NamedCharStatList_t, 8> vMPPropertyLists{
    {{"Apartments / Garages",
     ...
     {"Military / Smuggling", {{"PROP_DEFUNCBASE", "Bunker / Facility", StatDataType_t::INT, 0, 200}, {"PROP_HANGAR", "Hangar", StatDataType_t::INT, 0, 10}}},
     {"Entertainment",
...
{
    return statName == "STAMINA" || statName == "STRENGTH" || statName == "LUNG_CAPACITY" || statName == "WHEELIE_ABILITY" || statName == "FLYING_ABILITY" || statName == "SHOOTING_ABILITY" ||
           statName == "STEALTH_ABILITY" || statName == "SPECIAL_ABILITY" || statName == "SPECIAL_ABILITY_UNLOCKED";
}
...
void SPStatsManagerMenu()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        AddOption(charNames[i].second, bGoToCharacterPressed, nullFunc, SUB::SPSTATMANAGER_INCHAR);
