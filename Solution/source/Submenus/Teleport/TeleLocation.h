...
class TeleLocation
{
  ...
  public:
    TeleLocation(bool pbOnTheLine, float pX, float pY, float pZ, const std::string& pName) : name(pName), x(pX), y(pY), z(pZ), bOnTheLine(pbOnTheLine), bOffTheLine(false), bInterior(false)
    {
    }
    ...
    TeleLocation(
        ...
        float pY,
        float pZ,
        std::vector<std::string_view> pIplsToLoad,
        std::vector<std::string_view> pIplsToRemove,
        bool pbOnTheLine = false,
        bool pbOffTheLine = false,
        bool pbInterior = false
    )
    ...
    TeleLocation(
        ...
        std::vector<std::string_view> pIplsToLoad,
        std::vector<std::string_view> pIplsToRemove,
        std::vector<std::string_view> pInteriorProps,
        bool pbOnTheLine = false,
        bool pbOffTheLine = false,
        bool pbInterior = false
    )
        : name(pName), x(pX), y(pY), z(pZ), iplsToLoad(pIplsToLoad), iplsToRemove(pIplsToRemove), interiorProps(pInteriorProps), bOnTheLine(pbOnTheLine), bOffTheLine(pbOffTheLine),
