...
const std::map<UINT8, YachtBmp> vOptionNames{{1, {"The Orion", "yacht_model_0_0"}}, {2, {"The Pisces", "yacht_model_1_0"}}, {3, {"The Aquarius", "yacht_model_2_0"}}};
const std::vector<std::string> vFlagSuffixes{
    {"Argentina"}, {"Australia"},   {"Austria"},     {"Belgium"},    {"Brazil"},  {"Canadat_yt"}, {"China"},       {"Columbia"}, {"Croatia"},    {"CzechRep"},  {"Denmark"},     {"England"},
    {"EU_yt"},     {"Finland"},     {"France"},      {"German_yt"},  {"Hungary"}, {"Ireland"},    {"Israel"},      {"Italy"},    {"Jamaica"},    {"Japan_yt"},  {"Lstein"},      {"Malta"},
    {"Mexico_yt"}, {"Netherlands"}, {"NewZealand"},  {"Nigeria"},    {"Norway"},  {"Palestine"},  {"Poland"},      {"Portugal"}, {"PuertoRico"}, {"Russia_yt"}, {"Scotland_yt"}, {"Script"},
    {"Slovakia"},  {"Slovenia"},    {"SouthAfrica"}, {"Southkorea"}, {"Spain"},   {"Sweden"},     {"Switzerland"}, {"Turkey"},   {"UK_yt"},      {"US_yt"},     {"Wales"},
};
...
static GTAprop AttachYachtProp(Model model, GTAentity parent, const Vector3& offset, const Vector3& rotation, YachtBuildInfoStructure& yachtInfo, const YachtPropCfg& cfg = {})
{
    ...
    if (cfg.detachAfterAttach)
        p.Detach();
    if (cfg.freezeAfterAttach)
...
void CreateYacht(YachtBuildInfoStructure& yachtInfo)
{
    ...
    const std::vector<std::pair<Vector3, Vector3>> doorOffsets = {
        {{0.01894f, -3.3871f, 6.6600f}, {0.0f, 0.0f, 90.2950f}}, {{0.0046f, -0.6018f, 6.6600f}, {0.0f, 0.0f, -89.7050f}}, {{-36.8202f, -1.2778f, 0.6500f}, {0.0f, 0.0f, -89.9550f}}
    };
    ...
    static const std::vector<Vector3> buoyOffsets = {{-53.4131f, -10.0086f, -6.0113f}, {-1631.3173f, -1819.5632f, -0.6474f}, {-58.8841f, 0.2735f, -6.1121f}, {-1619.7332f, -1820.7730f, -0.8027f}};
    ...
    static const std::map<UINT8, std::vector<std::tuple<Model, Vector3, Vector3>>> vehicleOffsets = {
        {1, {{VEHICLE_TROPIC2, {-54.3528f, -13.3907f, -5.1819f}, {1.6733f, 1.9946f, -109.0565f}}, {VEHICLE_SEASHARK3, {-61.5043f, -8.9306f, -5.5869f}, {4.1636f, -0.8436f, 116.3082f}}}},
        {2,
                    ...
                    if (!bMyPedIsInVehicle)
                    {
                        if (myPos.DistanceTo(p.first) < scale)
                        {
                            ...
                            TASK_GO_STRAIGHT_TO_COORD(
                                0, outsideMarkerPos.x, outsideMarkerPos.y, outsideMarkerPos.z, 1.5f, 2000, Vector3::DirectionToRotation(Vector3::Normalize(outsideMarkerPos - p.second)).z, 0.0f
                            );
                            sq.Close();
