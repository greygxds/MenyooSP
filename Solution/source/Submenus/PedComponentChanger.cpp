...
bool IsWardrobeSubmenu(int submenu)
{
    ...
    case SUB::PED_HEADFEATURES_FACEGENERATOR:
...
void ApplyFraming()
{
    if (!camera.Exists() || !framedPed.Exists())
        return;
...
void TickExit()
{
    ...
    exitingCamera.SetPosition(Vector3(exitStartPos.x + (targetPos.x - exitStartPos.x) * s, exitStartPos.y + (targetPos.y - exitStartPos.y) * s, exitStartPos.z + (targetPos.z - exitStartPos.z) * s));
    exitingCamera.SetRotation(Vector3(
        exitStartRot.x + WrapDegrees(targetRot.x - exitStartRot.x) * s, exitStartRot.y + WrapDegrees(targetRot.y - exitStartRot.y) * s, exitStartRot.z + WrapDegrees(targetRot.z - exitStartRot.z) * s
    ));
...
void Tick()
{
    ...
    if (GetTickCount() - enabledAt > takeoverGraceMs && !camera.IsInterpolating() && World::GetRenderingCamera().Handle() != camera.Handle())
    {
        Disable(false);
    }
}
...
{
    bool pressed = false;
    AddOption(text, pressed, nullFunc, SUB::COMPONENTSPROPS2, true, true);
    if (pressed)
    {
...
void ComponentChanger()
{
    ...
    bool bRandomComponents = 0, frontView = 0, bDefaultComponents = 0, bClearAll = 0, ComponentChanger_online_police_m = 0, ComponentChanger_online_robber_m = 0, ComponentChanger_online_garbage_m = 0,
         ComponentChanger_online_police_f = 0, ComponentChanger_offline_police_michael = 0, ComponentChanger_offline_firefighter_michael = 0;
    ...
    {
        if (GET_NUMBER_OF_PED_DRAWABLE_VARIATIONS(g_activePedHandle, i) > 0)
            AddPedComponentOption(components[i], i);
    }
...
void ComponentChanger2()
{
    ...
    if (collectionModified || globalDrawableId != prevGlobalDrawableId || textureId != prevTextureId)
    {
        ...
        while (!HasPedSpecificDrawable(globalDrawableId))
        {
            if (goingForward)
            {
                if (globalDrawableId < maxGlobalDrawableId)
                    globalDrawableId++;
                else
                    globalDrawableId = 0;
            }
            ...
            {
                if (globalDrawableId > 0)
                    globalDrawableId--;
                else
                    globalDrawableId = maxGlobalDrawableId;
            }
            textureId = 0;
            SET_PED_COMPONENT_VARIATION(g_activePedHandle, s_selectedComponentIndex, globalDrawableId, textureId, 0);
        }
...
void ComponentChangerProps_()
{
    ...
    const std::vector<std::string> propNames{
        "Hats ~c~[p_head]", "Glasses ~c~[p_eyes]", "Ear Pieces ~c~[p_ears]", "Unknown 3", "Unknown 4", "Unknown 5", "Watches ~c~[p_lwrist]", "Bangles ~c~[p_rwrist]", "Unknown 8", "Unknown 9"
    };
    ...
    if (collectionModified || propTypeCurrent != propTypeOld || propTextureCurrent != propTextureOld)
    {
        ...
        else
        {
            ...
            while (!HasPedSpecificPropType(propTypeCurrent))
            {
                if (goingForward)
                {
                    if (propTypeCurrent < maxGlobalPropId)
                        propTypeCurrent++;
                    else
                        propTypeCurrent = -1;
                }
                else
                {
                    if (propTypeCurrent > -1)
                        ...
                        propTypeCurrent = maxGlobalPropId;
                }
                propTextureCurrent = 0;
                SET_PED_PROP_INDEX(g_activePedHandle, s_selectedPropIndex, propTypeCurrent, propTextureCurrent, NETWORK_IS_GAME_IN_PROGRESS(), 0);
            }
...
void Sub_Decals_InZone()
{
    ...
    if (Menu::OnSubBack == nullptr)
    {
        Menu::OnSubBack = [] { PedDecals::ClearPreviewTattoo(); };
    }
...
const std::vector<std::pair<std::string, std::vector<std::string>>> vCaptions_headOverlays{
    ...
    {/*"Beard"*/ "FACE_F_BEARD",
     {"HAIR_BEARD1",
      ...
      "HAIR_BEARD17",
      "HAIR_BEARD18",
      "HAIR_BEARD19"}}, // Beard HAIR_OPTION_0
    {/*"Eyebrows"*/ "FACE_F_EYEBR",
     {"CC_EYEBRW_0",  "CC_EYEBRW_1",  "CC_EYEBRW_2",  "CC_EYEBRW_3",  "CC_EYEBRW_4",  "CC_EYEBRW_5",  "CC_EYEBRW_6",  "CC_EYEBRW_7",  "CC_EYEBRW_8",  "CC_EYEBRW_9",  "CC_EYEBRW_10", "CC_EYEBRW_11",
      "CC_EYEBRW_12", "CC_EYEBRW_13", "CC_EYEBRW_14", "CC_EYEBRW_15", "CC_EYEBRW_16", "CC_EYEBRW_17", "CC_EYEBRW_18", "CC_EYEBRW_19", "CC_EYEBRW_20", "CC_EYEBRW_21", "CC_EYEBRW_22", "CC_EYEBRW_23",
      "CC_EYEBRW_24", "CC_EYEBRW_25", "CC_EYEBRW_26", "CC_EYEBRW_27", "CC_EYEBRW_28", "CC_EYEBRW_29", "CC_EYEBRW_30", "CC_EYEBRW_31", "CC_EYEBRW_32", "CC_EYEBRW_33"}},
    {/*"Wrinkles"*/ "FACE_F_SKINA",
     {"Crow's Feet",
      "First Signs",
      "Middle Aged",
      "Worry Lines",
      ...
      "Junkie",
      "Geriatric"}},
    {/*"Makeup & Face Paint"*/ "HAIR_OPTION_2",
     {"CC_MKUP_0",  "CC_MKUP_1",  "CC_MKUP_2",  "CC_MKUP_3",  "CC_MKUP_4",  "CC_MKUP_5",  "CC_MKUP_6",  "CC_MKUP_7",  "CC_MKUP_8",  "CC_MKUP_9",  "CC_MKUP_10",
      "CC_MKUP_11", "CC_MKUP_12", "CC_MKUP_13", "CC_MKUP_14", "CC_MKUP_15", "CC_MKUP_16", "CC_MKUP_17", "CC_MKUP_18", "CC_MKUP_19", "CC_MKUP_20", "CC_MKUP_21",
      "CC_MKUP_22", "CC_MKUP_23", "CC_MKUP_24", "CC_MKUP_25", "CC_MKUP_26", "CC_MKUP_27", "CC_MKUP_28", "CC_MKUP_29", "CC_MKUP_30", "CC_MKUP_31", "CC_MKUP_32",
      "CC_MKUP_33", "CC_MKUP_34", "CC_MKUP_35", "CC_MKUP_36", "CC_MKUP_37", "CC_MKUP_38", "CC_MKUP_39", "CC_MKUP_40", "CC_MKUP_41"}}, // Makeup & face paint HAIR_OPTION_2
    ...
    {/*"Pigment 2 - Blemishes"*/ "FACE_F_SKINB",
     {"Measles", "Pimples", "Spots",     "Break Out", "Blackheads", "Build Up", "Pustules", "Zits",          "Full Acne",          "Acne",       "Cheek Rash", "Face Rash", "Picker",
      "Puberty", "Eyesore", "Chin Rash", "Two Face",  "T Zone",     "Greasy",   "Marked",   "Acne Scarring", "Full Acne Scarring", "Cold Sores", "Impetigo"}},
    {/*"Lipstick"*/ "FACE_F_LIPST",
     {"CC_LIPSTICK_0", "CC_LIPSTICK_1", "CC_LIPSTICK_2", "CC_LIPSTICK_3", "CC_LIPSTICK_4", "CC_LIPSTICK_5", "CC_LIPSTICK_6", "CC_LIPSTICK_7", "CC_LIPSTICK_8", "CC_LIPSTICK_9"}},
    {/*"Spots"*/ "FACE_F_MOLE",
     {"Cherub",
      "All Over",
      "Irregular",
...
const std::vector<std::string> vCaptions_facialFeatures{{"Nose Width"}, {"Nose Bottom Height"}, {"Nose Tip Length"}, {"Nose Bridge Depth"}, {"Nose Tip Height"}, {"Nose Broken"},   {"Brow Height"},
                                                        {"Brow Depth"}, {"Cheekbone Height"},   {"Cheekbone Width"}, {"Cheek Depth"},       {"Eye Size"},        {"Lip Thickness"}, {"Jaw Width"},
                                                        {"Jaw Shape"},  {"Chin Height"},        {"Chin Depth"},      {"Chin Width"},        {"Chin Indent"},     {"Neck Width"}};
#pragma endregion
...
void Sub_Main()
{
    ...
    if (headBlend.shapeFirstID < 0 || headBlend.shapeFirstID > maxIds || headBlend.shapeSecondID < 0 || headBlend.shapeSecondID > maxIds || headBlend.shapeThirdID < 0 ||
        headBlend.shapeThirdID > maxIds || headBlend.skinFirstID < 0 || headBlend.skinFirstID > maxIds || headBlend.skinSecondID < 0 || headBlend.skinSecondID > maxIds || headBlend.skinThirdID < 0 ||
        headBlend.skinThirdID > maxIds)
    {
...
void Sub_FaceGenerator()
{
    ...
    bool bRandFace = false, bRandShapes = false, bRandSkins = false, bRandAllFeatures = false, bResetAllFeatures = false, bRandEverything = false;
    ...
    auto randomizeMix = []() -> float { return GET_RANDOM_FLOAT_IN_RANGE(0.0f, 1.0f); };
...
bool Apply(GTAped ep, const std::string& filePath, bool applyModelAndHead, bool applyProps, bool applyComps, bool applyDecals, bool applyDamageTextures, bool applyAttachedEntities)
{
    ...
    if (applyModelAndHead)
    {
        if (ep.Handle() == PLAYER_PED_ID())
        {
            ...
            ep = PLAYER_PED_ID();
            if (bWas241)
                g_activePedHandle = ep.Handle();
        }

        if (nodePedStuff.child("HasShortHeight").text().as_bool())
            SET_PED_CONFIG_FLAG(ep.Handle(), ePedConfigFlags::_Shrink, 1);
...
void ComponentChanger_Outfits()
{
    ...
    if (attachmentsPlus)
    {
        if (persistentAttachmentsTexterIndex < 2)
            persistentAttachmentsTexterIndex++;
    }
    if (attachmentsMinus)
    {
        if (persistentAttachmentsTexterIndex > 0)
            persistentAttachmentsTexterIndex--;
    }
    ...
    AddOptionDescription(
        "New XML Format saves the file with more readable node names. Legacy XML format saves it with non-human readable node names, but will more likely work with external XML importers."
    );
    ...
    if (!fileNames.empty())
    {
        ...
        for (auto& fileName : fileNames)
        {
            ...
            if (!searchStr.empty())
            {
                if (boost::to_upper_copy(fileName).find(searchStr) == std::string::npos)
                    continue;
            }
...
void ComponentChanger_Outfits2()
{
    ...
    bool outfits2_apply = false, outfits2_applyAllFeatures = false, outfits2_applyModel = false, outfits2_overwrite = false, outfits2_applySetDefault = false, outfits2_rename = false,
         outfits2_delete = false;
    ...
    if (outfits2_rename)
    {
        ...
        if (newName.length() > 0)
        {
            ...
            Game::Print::PrintErrorInvalidInput(newName);
    ...
    if (doc.load_file((const char*)filePath.c_str()).status == pugi::status_ok)
    {
        ...
        if (nodeAddAttachmentsToSpoonerDb)
        {
            ...
            AddTickol(
                "Persistent Attachments (AddToSpoonerDb)",
                bAddAttachemntsToSpoonerDb,
                bToggleAddAttachmentsToSpoonerDbPressed,
                bToggleAddAttachmentsToSpoonerDbPressed,
                TICKOL::BOXTICK,
                TICKOL::BOXBLANK
            );
...
void ComponentChanger_DefaultOutfits()
{
    ...
    if (modelHash == GET_HASH_KEY("Player_Zero"))
    {
        charType = 0;
    }
    else if (modelHash == GET_HASH_KEY("Player_One"))
    {
        charType = 1;
    }
    else if (modelHash == GET_HASH_KEY("Player_Two"))
    {
        charType = 2;
    }
    else if (modelHash == GET_HASH_KEY("MP_M_Freemode_01"))
    {
        charType = 3;
    }
    else if (modelHash == GET_HASH_KEY("MP_F_Freemode_01"))
    {
        charType = 4;
    }
    ...
    for (const auto& outfit : g_defaultOutfitCache)
    {
        if (!g_defaultOutfitSearch.empty())
        {
            ...
            if (lowerName.find(g_defaultOutfitSearch) == std::string::npos && lowerGxt.find(g_defaultOutfitSearch) == std::string::npos)
            {
                continue;
            }
        }
        ...
        if (outfitPressed)
        {
            ...
            addlog(
                ...
                "Applied Default Outfit: " + outfit.name + " | Hash: " + std::to_string(outfit.hash) + " | Comps: " + std::to_string(outfit.totalComponents) +
                    " | Props: " + std::to_string(outfit.totalProps)
            );
