...
unsigned int GetNumberOfBodyguardsSpawned(const EntityType& type)
{
    ...
    default:
        return static_cast<unsigned int>(std::count_if(BodyguardDb.begin(), BodyguardDb.end(), [type](const BodyguardEntity& item) { return item.Type == type; }));
    }
}
...
{
    auto it = std::remove_if(BodyguardDb.begin(), BodyguardDb.end(), [&](const BodyguardEntity& e) { return e.Handle.GetHandle() == ent.Handle.GetHandle(); });
