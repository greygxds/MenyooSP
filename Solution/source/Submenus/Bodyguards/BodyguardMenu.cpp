...
void BodyguardMainMenu()
{
    ...
    int blipIndex = 0;
    static const std::vector<std::pair<int, std::string>> blipOptions = {{1, "Standard"}, {280, "Friend"}, {480, "VIP"}};
    bool bBlipInput = false;
    bool bIcon_Plus = false, bIcon_Minus = false;
    bool oldGodmode = sub::BodyguardMenu::godmode;

    int formationIndex = 0;
    static const std::vector<std::pair<int, std::string>> formationOptions = {{0, "Default Formation"}, {1, "Circle (Inward)"}, {2, "Circle (North)"}, {3, "Line"}};
    ...
    if (bHealth_plus && sub::BodyguardMenu::health < INT_MAX)
        ++sub::BodyguardMenu::health;
    if (bHealth_minus && sub::BodyguardMenu::health > 0)
        --sub::BodyguardMenu::health;
    if (bHealth_input)
    {
    ...
    if (bArmor_plus && sub::BodyguardMenu::armor < INT_MAX)
        ++sub::BodyguardMenu::armor;
    if (bArmor_minus && sub::BodyguardMenu::armor > 0)
        --sub::BodyguardMenu::armor;
    if (bArmor_input)
    {
    ...
    if (bTeleportBodyguards)
    {
        ...
        if (ENTITY::DOES_ENTITY_EXIST(playerPed))
        {
            ...
            for (unsigned int i = 0; i < sub::BodyguardMenu::BodyguardDb.size(); ++i)
            {
                ...
                Vector3 targetPos = playerPos + (forward * baseDist) + Vector3(0.0f, 0.0f, 0.2f) + (forward * (spacing * placed));

                ENTITY::SET_ENTITY_COORDS_NO_OFFSET(ped, targetPos.x, targetPos.y, targetPos.z, false, false, false);
