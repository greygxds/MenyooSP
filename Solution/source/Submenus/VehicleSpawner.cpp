...
void DrawVehicleStats(const GTAmodel::Model& vehModel)
{
    ...
    if (statsCache.hash != hash)
    {
        ...
        if (make.empty() || make == "NOTFOUND")
            statsCache.makeName = "---";
        else
            statsCache.makeName = Game::GetGXTEntry(make, make);
...
void AddVehicleCategoryOption(const std::string& text, UINT8 index, bool* extra_option_code)
{
    ...
    {
    case OPENWHEEL:
        tempvecp = &g_vehHashes_OPENWHEEL;
        break;
    case SUPER:
        tempvecp = &g_vehHashes_SUPER;
        break;
    case SPORT:
        tempvecp = &g_vehHashes_SPORT;
        break;
    case SPORTSCLASSIC:
        tempvecp = &g_vehHashes_SPORTSCLASSIC;
        break;
    case COUPE:
        tempvecp = &g_vehHashes_COUPE;
        break;
    case MUSCLE:
        tempvecp = &g_vehHashes_MUSCLE;
        break;
    case OFFROAD:
        tempvecp = &g_vehHashes_OFFROAD;
        break;
    case SUV:
        tempvecp = &g_vehHashes_SUV;
        break;
    case SEDAN:
        tempvecp = &g_vehHashes_SEDAN;
        break;
    case COMPACT:
        tempvecp = &g_vehHashes_COMPACT;
        break;
    case VAN:
        tempvecp = &g_vehHashes_VAN;
        break;
    case SERVICE:
        tempvecp = &g_vehHashes_SERVICE;
        break;
    case TRAIN:
        tempvecp = &g_vehHashes_TRAIN;
        break;
    case EMERGENCY:
        tempvecp = &g_vehHashes_EMERGENCY;
        break;
    case MOTORCYCLE:
        tempvecp = &g_vehHashes_MOTORCYCLE;
        break;
    case BICYCLE:
        tempvecp = &g_vehHashes_BICYCLE;
        break;
    case PLANE:
        tempvecp = &g_vehHashes_PLANE;
        break;
    case HELICOPTER:
        tempvecp = &g_vehHashes_HELICOPTER;
        break;
    case BOAT:
        tempvecp = &g_vehHashes_BOAT;
        break;
    case INDUSTRIAL:
        tempvecp = &g_vehHashes_INDUSTRIAL;
        break;
    case COMMERCIAL:
        tempvecp = &g_vehHashes_COMMERCIAL;
        break;
    case UTILITY:
        tempvecp = &g_vehHashes_UTILITY;
        break;
    case MILITARY:
        tempvecp = &g_vehHashes_MILITARY;
        break;
    case OTHER:
        tempvecp = &g_vehHashes_OTHER;
        break;
    case DRIFT:
        tempvecp = &g_vehHashes_DRIFT;
        break;
    }
    if (tempvecp->empty())
    {
...
void AddVehicleSpawnOption(const std::string& text, const GTAmodel::Model& vehModel, Ped ped)
{
    ...
    if (pressed)
    {
        ...
        SetVehicleMaxUpgrades(
            vehicle,
            g_spawnVehicleAutoUpgrade,
            g_spawnVehicleInvincible,
            g_spawnVehiclePlateType,
            g_spawnVehiclePlateTexterValue == 0 ? g_spawnVehiclePlateText : "",
            g_spawnVehicleNeonToggle,
            g_spawnVehicleNeonColor.R,
            g_spawnVehicleNeonColor.G,
            g_spawnVehicleNeonColor.B,
            g_spawnVehiclePrimaryColor,
            g_spawnVehicleSecondaryColor
        );
...
void SpawnVehicleMenu()
{
    ...
    if (spawnRandom || spawnVehicleInput)
    {
        ...
        if (model.IsInCdImage() && model.IsVehicle())
        {
            ...
            Game::Print::PrintErrorInvalidModel(model.VehicleModelName());
    }
}
...
void RebuildResults(const std::string& searchStr)
{
    ...
    for (auto& model : g_vehHashes)
    {
        ...
        if (typeFilter != 0)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                pass = model.IsBike() || model.IsBicycle();
                break;
            case 3:
                pass = model.IsHeli();
                break;
        ...
        if (!searchUpper.empty())
        {
            ...
            if (displayUpper.find(searchUpper) == std::string::npos && modelUpper.find(searchUpper) == std::string::npos && makeUpper.find(searchUpper) == std::string::npos)
                continue;
        }
    ...
    {
    case 0:
        std::sort(results.begin(), results.end(), [](const Model& a, const Model& b) { return boost::to_lower_copy(a.VehicleDisplayName(true)) < boost::to_lower_copy(b.VehicleDisplayName(true)); });
        break;
    case 1:
        std::sort(results.begin(), results.end(), [](const Model& a, const Model& b) { return GET_VEHICLE_MODEL_ESTIMATED_MAX_SPEED(a.hash) > GET_VEHICLE_MODEL_ESTIMATED_MAX_SPEED(b.hash); });
        break;
    case 2:
        std::sort(results.begin(), results.end(), [](const Model& a, const Model& b) { return GET_VEHICLE_MODEL_ACCELERATION(a.hash) > GET_VEHICLE_MODEL_ACCELERATION(b.hash); });
        break;
    case 3:
        std::sort(
            results.begin(),
            results.end(),
            [&priceTable](const Model& a, const Model& b)
            {
...
void SpawnVehicleSearchMenu()
{
    ...
    if (sortRight && sortIndex < (int)(sortOptions.size() - 1))
        sortIndex++;
    if (sortLeft && sortIndex > 0)
        sortIndex--;

    bool classRight = false, classLeft = false;
    AddTexter("Class", classFilter, classOptions, null, classRight, classLeft);
    if (classRight && classFilter < (int)(classOptions.size() - 1))
        classFilter++;
    if (classLeft && classFilter > 0)
        classFilter--;

    bool typeRight = false, typeLeft = false;
    AddTexter("Type", typeFilter, typeOptions, null, typeRight, typeLeft);
    if (typeRight && typeFilter < (int)(typeOptions.size() - 1))
        typeFilter++;
    if (typeLeft && typeFilter > 0)
        typeFilter--;

    if (dirty || searchStr != lastSearch || sortIndex != lastSort || classFilter != lastClass || typeFilter != lastType)
    {
    ...
    for (auto& vehModel : results)
    {
        ...
        if (Menu::IsLastDrawnOptionSelected())
        {
            ...
            if (Menu::usingControllerInput)
            {
                ...
                {
                    !bIsAFav ? SpawnVehicleAddVehicleModelToFavourites(vehModel, Game::InputBox("", 28U, "Enter custom name:", vehModel.VehicleDisplayName(true)))
                             : SpawnVehicleRemoveVehicleModelFromFavourites(vehModel);
                }
            }
            else
            {
...
void SpawnVehicleOptions()
{
    ...
    std::vector<std::string> plateTypeNames{
        "CMOD_PLA_0", "CMOD_PLA_4", "CMOD_PLA_3", "CMOD_PLA_1", "CMOD_PLA_2", "Yankton", "CMOD_PLA_6", "CMOD_PLA_7", "CMOD_PLA_8", "CMOD_PLA_9", "CMOD_PLA_10", "CMOD_PLA_11", "CMOD_PLA_12"
    }; // BOW1, YOBLA, YOBLU, BOW2, BOW3, YANKTON, ECOLA, LASVENTURAS, LIBERTYCITY, LSCARMEET, LSPOUNDERS, SPRUNK
...
void SpawnVehicleAllCategoriesMenu()
{
    ...
    {
    case OPENWHEEL:
        tempVehicleCopy = &g_vehHashes_OPENWHEEL;
        break;
    case SUPER:
        tempVehicleCopy = &g_vehHashes_SUPER;
        break;
    case SPORT:
        tempVehicleCopy = &g_vehHashes_SPORT;
        break;
    case SPORTSCLASSIC:
        tempVehicleCopy = &g_vehHashes_SPORTSCLASSIC;
        break;
    case COUPE:
        tempVehicleCopy = &g_vehHashes_COUPE;
        break;
    case MUSCLE:
        tempVehicleCopy = &g_vehHashes_MUSCLE;
        break;
    case OFFROAD:
        tempVehicleCopy = &g_vehHashes_OFFROAD;
        break;
    case SUV:
        tempVehicleCopy = &g_vehHashes_SUV;
        break;
    case SEDAN:
        tempVehicleCopy = &g_vehHashes_SEDAN;
        break;
    case COMPACT:
        tempVehicleCopy = &g_vehHashes_COMPACT;
        break;
    case VAN:
        tempVehicleCopy = &g_vehHashes_VAN;
        break;
    case SERVICE:
        tempVehicleCopy = &g_vehHashes_SERVICE;
        break;
    case TRAIN:
        tempVehicleCopy = &g_vehHashes_TRAIN;
        break;
    case EMERGENCY:
        tempVehicleCopy = &g_vehHashes_EMERGENCY;
        break;
    case MOTORCYCLE:
        tempVehicleCopy = &g_vehHashes_MOTORCYCLE;
        break;
    case BICYCLE:
        tempVehicleCopy = &g_vehHashes_BICYCLE;
        break;
    case PLANE:
        tempVehicleCopy = &g_vehHashes_PLANE;
        break;
    case HELICOPTER:
        tempVehicleCopy = &g_vehHashes_HELICOPTER;
        break;
    case BOAT:
        tempVehicleCopy = &g_vehHashes_BOAT;
        break;
    case INDUSTRIAL:
        tempVehicleCopy = &g_vehHashes_INDUSTRIAL;
        break;
    case COMMERCIAL:
        tempVehicleCopy = &g_vehHashes_COMMERCIAL;
        break;
    case UTILITY:
        tempVehicleCopy = &g_vehHashes_UTILITY;
        break;
    case MILITARY:
        tempVehicleCopy = &g_vehHashes_MILITARY;
        break;
    case OTHER:
        tempVehicleCopy = &g_vehHashes_OTHER;
        break;
    case DRIFT:
        tempVehicleCopy = &g_vehHashes_DRIFT;
        break;
    }
    ...
    if (nodeRoot.first_child())
    {
        ...
        for (auto nodeLocToLoad = nodeRoot.first_child(); nodeLocToLoad; nodeLocToLoad = nodeLocToLoad.next_sibling())
        {
            ...
            else
            {
                ...
                case SUB::MSENGINESOUND:
                    Vehicle engineSoundVehicleTarget = GetVehicleModShopTarget();
                    AddTickol(vehDisplayName, GetVehicleEngineSoundName(engineSoundVehicleTarget) == vehModel.VehicleDisplayName(false), setEngineSoundPressed, setEngineSoundPressed);
                    if (setEngineSoundPressed)
                    {
                        GTAvehicle veh12 = engineSoundVehicleTarget;
                        veh12.RequestControl(200);
...
const std::vector<std::string> CAPTIONS_CUNNINGSTUNTS{
    "Nagasaki BF400",
    "Grotti Brioso R/A",
    "Vapid Desert Raid",
    "Emperor ETR1",
...
const std::vector<std::string> CAPTIONS_BIKERSUPDATE{
    "LCC Avarus",       "Nagasaki Chimera",         "Shitzu Defiler",    "Pegassi Faggio Mod",    "Pegassi Faggio Sport",   "Shitzu Hakuchou Drag Bike", "Maibatsu Manchez", "Western Nightblade",
    "Western Rat Bike", "Nagasaki Street Blazer",   "Western Wolfsbane", "Western Zombie Bobber", "Western Zombie Chopper", "Western Daemon (Custom)",   "BF Raptor",        "Pegassi Vortex",
    "LCC Sanctus",      "Declasse Tornado Rat Rod", "Nagasaki Shotaro",  "Pegassi Esskey",        "Bravado Youga Classic"
};

const std::vector<std::string> CAPTIONS_IMPORTEXPORT{
    "Brute Armored Boxville", "Nagasaki Blazer Aqua",    "Principe Diabolus",    "Principe Diabolus (Custom)", "Annis Elegy Retro (Custom)", "Ocelot Penetrator",
    "JoBuilt Phantom Wedge",  "BF Ramp Buggy",           "Coil Rocket Voltic",   "Imponte Ruiner 2000",        "Karin Technical Aqua",       "Pegassi Tempesta",
    "MTL Wastelander",        "Truffade Nero",           "Truffade Nero Custom", "Pfister Comet Retro Custom", "Pegassi FCR 1000",           "Pegassi FCR 1000 Custom",
    "Progen Itali GTB",       "Progen Itali GTB Custom", "Dewbauchee Specter",   "Dewbauchee Specter Custom"
};
...
const std::vector<std::string> CAPTIONS_GUNRUNNING{
...
const std::vector<std::string> CAPTIONS_SMUGGLERSRUN{"Buckingham Alpha-Z1",   "Nagasaki Havok",
                                                     "LF-22 Starling",        "Vapid Retinue",
                                                     "Western Company Rogue", "Mammoth Tula",
                                                     "Nagasaki Ultralight",   "V-65 Molotok",
                                                     "Grotti Visione",        "Dewbauchee Rapid GT Classic",
                                                     "RM-10 Bombushka",       "Buckingham Howard NX-25",
                                                     "Mammoth Mogul",         "Buckingham Pyro",
                                                     "Grotti Vigilante",      "Western Company Seabreeze",
                                                     "P-45 Nokota",           "Coil Cyclone",
                                                     "FH-1 Hunter",           "JoBuilt P-996 LAZER"};

const std::vector<std::string> CAPTIONS_DOOMSDAYHEIST{
    "Akula",
    "Mammoth Avenger",
    "HVY Barrage",
    "HVY Chernobog",
...
const std::vector<std::string> CAPTIONS_SOUTHERNSASPORTSERIES{
    "Overflod Entity XXR",
    "Vulcar Fagaloa",
    "Vapid GB200",
...
const std::vector<std::string> CAPTIONS_AFTERHOURS{
    "Festival Bus",
...
const std::vector<std::string> CAPTIONS_DIAMONDCASINO{
    "Obey 8F Drafter",
    "Vapid Caracara 4x4",
    "Weeny Issi Sport",
    "Enus Paragon R",
...
const std::vector<std::string> CAPTIONS_LSSUMMERSPECIAL{
    "Imponte Beater Dukes",
    "Benefactor BR8 (Formula 1 Car)",
    "BF Club",
    "Invetero Coquette D10",
    "Declasse DR1 (IndyCar)",
...
const std::vector<std::string> CAPTIONS_CAYOHEIST{
...
const std::vector<std::string> CAPTIONS_LSTUNERS{
    "Karin Calico GTF",
    "Vapid Dominator GTT",
    "Annis Euros",
    "Karin Futo GTX",
...
const std::vector<std::string> CAPTIONS_THECONTRACT{
    "Pfister Astron",
    "Bravado Buffalo STX",
    "Dewbauchee Champion",
...
const std::vector<std::string> CAPTIONS_CRIMINALENTERPRISES{
    "Obey 10F",
    "Obey 10F Widebody",
...
const std::vector<std::string> CAPTIONS_THECHOPSHOP{
    "Vapid Aleutian",
    "Karin Asterope GZ",
    "Vapid Dominator GT",
    "Fathom FR36 (Drift Car)",
    "Declasse Impaler LX",
    "Brute Police Riot",
    "Vapid Police Stanier LE Cruiser",
    "Grotti Turismo Omaggio",
    "Vapid Unmarked Cruiser",
    "Declasse Vigero ZX Convertible (HSW Upgrade)",
    "Karin Vivanite",
...
const std::vector<std::string> CAPTIONS_BOTTOMDOLLAR{
    "Impaler LX Cruiser",
...
const std::vector<std::string> VALUES_CUNNINGSTUNTS{
    "bf400", "brioso", "trophytruck2", "sheava", "gargoyle", "omnis", "le7b", "trophytruck", "tropos", "tyrus", "cliffhanger", "lynx", "tampa2", "rallytruck", "contender"
};

const std::vector<std::string> VALUES_BIKERSUPDATE{"avarus",  "chimera", "defiler", "faggio3", "faggio", "hakuchou2", "manchez",  "nightblade", "ratbike", "blazer4", "wolfsbane",
                                                   "zombiea", "zombieb", "daemon2", "raptor",  "vortex", "sanctus",   "tornado6", "shotaro",    "esskey",  "youga2"};

const std::vector<std::string> VALUES_IMPORTEXPORT{"boxville5", "blazer5",     "diablous", "diablous2", "elegy",  "penetrator", "phantom2", "dune5",    "voltic2",   "ruiner2", "technical2",
                                                   "tempesta",  "wastelander", "nero",     "nero2",     "comet3", "fcr",        "fcr2",     "italigtb", "italigtb2", "specter", "specter2"};

const std::vector<std::string> VALUES_CUNNINGSTUNTS2{"gp1", "ruston", "infernus2", "turismo2", "dukes2"};

const std::vector<std::string> VALUES_GUNRUNNING{
    "trailersmall2",
    "apc",
    "caddy3",
    "dune3",
...
const std::vector<std::string> VALUES_SMUGGLERSRUN{"alphaz1",   "havok",  "starling", "retinue", "rogue",     "tula",      "microlight", "molotok", "visione", "rapidgt3",
                                                   "bombushka", "howard", "mogul",    "pyro",    "vigilante", "seabreeze", "nokota",     "cyclone", "hunter",  "lazer"};

const std::vector<std::string> VALUES_DOOMSDAYHEIST{"akula",    "avenger",   "barrage",  "chernobog", "comet4",  "deluxo",   "pariah",    "raiden",   "riot2",  "riata",
                                                    "sc1",      "stromberg", "thruster", "khanjali",  "volatol", "yosemite", "sentinel3", "streiter", "hermes", "autarch",
                                                    "savestra", "viseris",   "gt500",    "revolter",  "neon",    "kamacho",  "hustler",   "z190",     "comet5"};

const std::vector<std::string> VALUES_SOUTHERNSASPORTSERIES{
    "entity2", "fagaloa", "GB200", "hotring", "taipan", "ellie", "Tezeract", "caracara", "flashgt", "issi3", "seasparrow", "dominator3", "tyrant", "cheburek", "michelli"
};

const std::vector<std::string> VALUES_AFTERHOURS{
    "pbus2", "jester3", "mule4", "mule4", "pounder2", "speedo4", "swinger", "blimp3", "stafford", "strikeforce", "terbyte", "oppressor2", "scramjet", "menacer", "freecrawler"
};

const std::vector<std::string> VALUES_ARENAWAR{"bruiser",  "brutus", "cerberus", "deathbike", "dominator4", "impaler", "impaler2", "imperator", "issi4",   "monster3", "scarab",
                                               "slamvan4", "zr380",  "toros",    "clique",    "italigto",   "tulip",   "schlagen", "rcbandito", "deviant", "vamos",    "deveste"};

const std::vector<std::string> VALUES_DIAMONDCASINO{"drafter", "caracara2", "issi7",   "paragon", "paragon2", "s80",     "thrax",   "neo",      "gauntlet3", "emerus", "nebula",
                                                    "locust",  "gauntlet4", "krieger", "zion3",   "hellion",  "jugular", "dynasty", "zorrusso", "rrocket",   "novak",  "peyote2"};

const std::vector<std::string> VALUES_LSSUMMERSPECIAL{
    "dukes3", "openwheel1", "club", "coquette4", "openwheel2", "gauntlet5", "glendale2", "landstalker2", "manana2", "penumbra2", "peyote3", "seminole2", "tigon", "yosemite3", "youga3"
};

const std::vector<std::string> VALUES_CAYOHEIST{"annihilator2", "avisa",    "kosatka", "patrolboat", "longfin",  "alkonost",  "seasparrow3", "toreador", "winky", "veto",
                                                "veto2",        "italirsx", "weevil",  "brioso2",    "manchez2", "slamtruck", "vetir",       "squaddie", "verus", "dinghy5"};

const std::vector<std::string> VALUES_LSTUNERS{
    "calico",
    "dominator8",
    "drifteuros",
    "futo2",
...
const std::vector<std::string> VALUES_THECONTRACT{
    "astron", "buffalo4", "champion", "cinquemila", "Deity", "Ignus", "jubilee", "youga4", "zeno", "baller7", "patriot3", "comet7", "shinobi", "iwagen", "granger2", "reever"
};

const std::vector<std::string> VALUES_CRIMINALENTERPRISES{"tenf",     "tenf2",     "brioso3", "conada",    "corsita", "draugur", "greenwood", "kanjosj", "lm87",  "omnisegt",
                                                          "postlude", "rhinehart", "ruiner4", "sentinel4", "sm722",   "torero2", "vigero2",   "weevil2", "sm722", "draugur",
                                                          "ruiner4",  "brioso3",   "vigero2", "postlude",  "kanjosj", "tenf",    "rhinehart", "weevil2", "tenf2", "sentinel4"};

const std::vector<std::string> VALUES_THECHOPSHOP{"aleutian", "asterope2", "dominator9", "fr36",   "impaler6",   "riot",        "police",   "turismo3", "police4", "vigero3",
                                                  "vivanite", "pranger",   "boxville6",  "dorado", "cavalcade3", "polgauntlet", "impaler5", "terminus", "baller8", "benson2"};

const std::vector<std::string> VALUES_BOTTOMDOLLAR{
    "polimpaler6",
    "poldominator10",
    "castigator",
    "pipistrello",
...
const std::vector<std::string> Captions_heistsDLC = {
    "Karin Kuruma", "Karin Kuruma (Armored)", "Armored Boxville", "Insurgent", "Insurgent Pickup", "Technical", "Casco", "Velum 5-Seater", "Hydra", "Savage", "Valkyrie", "Lectro", "Dinghy 4-Seater"
};
const std::vector<std::string> Values_heistsDLC = {"kuruma", "kuruma2", "boxville4", "insurgent", "insurgent2", "technical", "casco", "velum2", "hydra", "savage", "valkyrie", "lectro", "dinghy3"};
...
const std::vector<std::string> CAPTIONS_BEACH_BUM =
    { //done
        "BF Bifta",
        "Canis Kalahari",
        "Bravado Paradise",
...
const std::vector<std::string> VALUES_BEACH_BUM =
    { //done
        "bifta",
        "kalahari",
        "paradise",
...
const std::vector<std::string> CAPTIONS_LOW_RIDERS =
    { //done
        "Declasse Buccaneer",
        "Declasse Primo",
        "Imponte Chino",
...
const std::vector<std::string> VALUES_LOW_RIDERS =
    { //done
        "buccaneer",
        "primo",
        "chino",
...
const std::vector<std::string> CAPTIONS_HIGH_LIFE_ILL_GOT_ONE =
    { //dome
        "Enus Huntley S",
        "Dewbauchee Massacro",
        "Pegassi Zentorno",
...
const std::vector<std::string> VALUES_HIGH_LIFE_ILL_GOT_ONE =
    { //done
        "huntley",
        "massacro",
        "zentorno",
...
const std::vector<std::string> CAPTIONS_ILL_GOT_TWO =
    { //done
        "Pegassi Osiris",
        "Enus Windsor Drop",
...
const std::vector<std::string> VALUES_ILL_GOT_TWO =
    { //done
        "osiris",
        "windsor2",
        "schafter3",
...
const std::vector<std::string> CAPTIONS_HIGH_LIFE =
    { //done
        "Pegassi Zentorno",
        "Dewbauchee Massacro",
        "Enus Huntley S",
...
const std::vector<std::string> VALUES_HIGH_LIFE =
    { //done
        "zentorno",
        "massacro",
        "huntley",
...
const std::vector<std::string> CAPTIONS_NOT_HIPSTER =
    { //done
        "Benefactor Glendale",
        "Benefactor Panto",
        "Bravado Rhapsody",
...
const std::vector<std::string> VALUES_NOT_HIPSTER =
    { //done
        "glendale",
        "panto",
        "rhapsody",
...
const std::vector<std::string> CAPTIONS_INDEPENDENCE_DAY =
    { //dine
        "Western Sovereign",
        "Vapid Liberator"
};
const std::vector<std::string> VALUES_INDEPENDENCE_DAY =
    { //done
        "sovereign",
        "monster"
};

const std::vector<std::string> CAPTIONS_FLIGHT_SCHOOL =
    { //done
        "Western Besra",
        "Buckingham Miljet",
...
const std::vector<std::string> VALUES_FLIGHT_SCHOOL =
    { //done
        "besra",
        "miljet",
        "coquette2",
...
const std::vector<std::string> CAPTIONS_LAST_TEAM_STANDING =
    { //done
        "Lampadati Furore GT",
        "Shitzu Hakuchou",
        "LCC Innovation"
};
const std::vector<std::string> VALUES_LAST_TEAM_STANDING =
    { //done
        "furoregt",
        "hakuchou",
        "innovation"
...
const std::vector<std::string> CAPTIONS_FESTIVE_2014 =
    { //done
        "Bravado Rat-Truck",
...
const std::vector<std::string> VALUES_FESTIVE_2014 =
    { //done
        "ratloader2",
        "slamvan2",
        "jester2",
...
const std::vector<std::string> CAPTIONS_EXECUTIVES_CRIMINALS =
    { //done
...
const std::vector<std::string> VALUES_EXECUTIVES_CRIMINALS =
    { //done
        "banshee2",
        "btype3",
        "schafter5",
                ...
                if (IS_DISABLED_CONTROL_JUST_PRESSED(2, INPUT_SCRIPT_RLEFT))
                {
                    if (!bIsAFav)
                    {
                        SpawnVehicleAddVehicleModelToFavourites(
                            selectedCategory.values[vehDlcIdToSpawn], Game::InputBox("", 28U, "Enter custom name:", GTAmodel::Model(selectedCategory.values[vehDlcIdToSpawn]).VehicleDisplayName(true))
                        );
                    }
                    else
                    {
...
template <typename MapT, typename KeyT> static void writeMultiplierIfPresent(pugi::xml_node parent, const char* name, const MapT& map, const KeyT& key)
{
    auto it = map.find(key);
    if (it != map.end())
        parent.append_child(name).text() = it->second;
}

struct VehicleDoorEntry
{
    const char* xmlName;
    VehicleDoor door;
};
...
void VehicleSaveToFile(std::string filePath, GTAvehicle ev)
{
    ...
    for (UINT8 i = 0; i < 60; i++)
    {
        if (ev.DoesExtraExist(i))
            nodeVehicleModExtras.append_child(("_" + std::to_string(i)).c_str()).text() = ev.GetExtraOn(i);
    }
...
void VehicleReadFromFile(std::string filePath, GTAentity ped)
{
    ...
    case 0:
        break; // FileDecides
    case 1:
        bAddAttachmentsToSpoonerDb = false;
        break; // ForceOff
    case 2:
        bAddAttachmentsToSpoonerDb = true;
        break; // ForceOn
    }
...
int saveCarVars(GTAvehicle vehicle)
{
    ...
    int color1 = vehicle.GetPrimaryColour(), color2 = vehicle.GetSecondaryColour(), color3 = vehicle.GetPearlescentColour(), color4 = vehicle.GetRimColour(), color5 = vehicle.GetInteriorColour(),
...
void VehicleSaverMenu()
{
    ...
    AddTexter(
        "AddAttachmentsToSpoonerDB", _persistentAttachmentsTexterIndex, std::vector<std::string>{"FileDecides", "ForceOff", "ForceOn"}, null, persistentAttachments_plus, persistentAttachments_minus
    );
...
void VehSaverInItemMenu()
{
    ...
    if (doc.load_file((const char*)filePath.c_str()).status == pugi::status_ok)
    {
        ...
        AddTickol(
            "Persistent Attachments (AddToSpoonerDb)", bAddAttachemntsToSpoonerDb, bToggleAddAttachmentsToSpoonerDbPressed, bToggleAddAttachmentsToSpoonerDbPressed, TICKOL::BOXTICK, TICKOL::BOXBLANK
        );
