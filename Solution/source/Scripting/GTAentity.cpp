...
GTAentity::GTAentity(int handle) : mHandle(handle)
{
}
...
Vector3 GTAentity::GetScale() const
{
    ...
    {
        return {GTAmemory::ReadFloat(ptr + 0x60), GTAmemory::ReadFloat(ptr + 0x74), GTAmemory::ReadFloat(ptr + 0x88)};
    }
}
...
void GTAentity::SetScale(Vector3 value)
{
    ...
    if (IsVehicle())
    {
        ...
        auto apply = [](UINT64 m, float sx, float sy, float sz)
        {
            ...
            if (lr > 0.0001f)
                GTAmemory::WriteVector3(m, r * (sx / lr));
            if (lf > 0.0001f)
                GTAmemory::WriteVector3(m + 0x10, f * (sy / lf));
            if (lu > 0.0001f)
                GTAmemory::WriteVector3(m + 0x20, u * (sz / lu));
        };
        apply(ptr + 0x60, value.x, value.y, value.z);
        if (drawPtr)
            apply(drawPtr + 0x20, value.x, value.y, value.z);
...
void GTAentity::AttachTo(GTAentity entity, int boneIndex, Vector3 position, Vector3 rotation, bool b9, bool useSoftPinning, bool collision, bool isPed, int vertexIndex, bool fixedRot)
{
    ATTACH_ENTITY_TO_ENTITY(
        this->mHandle, entity.Handle(), boneIndex, position.x, position.y, position.z, rotation.x, rotation.y, rotation.z, b9, useSoftPinning, collision, isPed, vertexIndex, fixedRot, 0
    );
}
...
void GTAentity::Oscillate(const Vector3& position, float angleFreq, float dampRatio)
{
    ...
    this->ApplyForce(((position - this->GetPosition()) * (angleFreq * angleFreq)) - (2.0f * angleFreq * dampRatio * this->GetVelocity()) + Vector3(0.0f, 0.0f, 0.1f), ForceType::MaxForceRot2);
}
