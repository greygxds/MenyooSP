...
namespace Sound
{
GameSound::GameSound() : active(false), soundID(-1)
{
}

GameSound::GameSound(const std::string& nsoundSet, const std::string& nsound) : active(false), sound(nsound), soundSet(nsoundSet), soundID(-1)
...
{
    if (!activePrintBottomCentreMessage)
        return;
...
{
    activePrintBottomCentreMessage = PrintBottomCentreMessage{Language::TranslateToSelected(std::move(s)), GetTickCount() + static_cast<DWORD>((std::max)(0, time))};
}
...
namespace
{
enum class NotificationAnimation
{
    Entering,
    Visible,
    Exiting
...
std::vector<std::string> WrapText(const std::string& text, float maxWidth)
{
    ...
    {
        if (word.empty())
            wordStartFormatting = activeFormatting;
    };
    auto appendLine = [&](bool allowEmpty)
    {
        if (allowEmpty || !GetVisibleText(line).empty())
    ...
    for (std::string::size_type index = 0; index < text.length();)
    {
        ...
        if (text[index] == '~')
        {
            ...
            if (end != std::string::npos)
            {
                ...
                else if (IsPersistentFormattingTag(tag))
                    activeFormatting += tag;
                index = end + 1;
                continue;
            }
        }
...
std::string ResolveNotificationText(std::string text)
{
    ...
    if (!DOES_TEXT_LABEL_EXIST(text.c_str()))
        return text;
...
void PrepareNotification(QueuedNotification& notification, DWORD now)
{
    ...
    notification.panelHeight =
        topPadding + bottomPadding + (notification.title.has_value() ? titleLineHeight + titleDescriptionGap : 0.0f) + (notification.descriptionLines.size() * descriptionLineHeight);
...
void RenderNotification(DWORD now)
{
    ...
    if (MenuConfig::bShowNotificationBackground)
    {
        ...
        DRAW_RECT(
            panelX + stripeWidth / 2.0f, panelY + notification.panelHeight / 2.0f, stripeWidth, notification.panelHeight, titlebox.R, titlebox.G, titlebox.B, ApplyOpacity(titlebox.A, progress), false
        );
    }
    ...
    for (const auto& line : notification.descriptionLines)
    {
        Print::SetupDraw(
            font_options,
            Vector2(descriptionTextScale, descriptionTextScale),
            false,
            false,
            true,
            {optiontext.R, optiontext.G, optiontext.B, static_cast<UINT8>(ApplyOpacity(optiontext.A, progress))},
            {0.0f, textX + textWidth}
        );
