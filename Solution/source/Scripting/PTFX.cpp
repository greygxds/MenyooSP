...
LoopedPTFX::LoopedPTFX() : mHandle(-1)
{
}

LoopedPTFX::LoopedPTFX(const std::string& assetName, const std::string& fxName) : mHandle(-1), asset(assetName), effect(fxName)
{
}

LoopedPTFX::LoopedPTFX(const sFxData& newFxData) : mHandle(-1), asset(newFxData.asset), effect(newFxData.effect)
{
}
...
void LoopedPTFX::Start(const Vector3& position, float scale, const Vector3& rotation)
{
    ...
    this->mHandle = START_PARTICLE_FX_LOOPED_AT_COORD((PCHAR)this->effect.c_str(), position.x, position.y, position.z, rotation.x, rotation.y, rotation.z, scale, 0, 0, 0, 0);
}
...
NonLoopedPTFX::NonLoopedPTFX(const std::string& assetName, const std::string& fxName) : asset(assetName), effect(fxName)
{
}

NonLoopedPTFX::NonLoopedPTFX(const sFxData& newFxData) : asset(newFxData.asset), effect(newFxData.effect)
{
}
...
void NonLoopedPTFX::Start(const Vector3& position, float scale, const Vector3& rotation)
{
    ...
    START_NETWORKED_PARTICLE_FX_NON_LOOPED_AT_COORD((PCHAR)this->effect.c_str(), position.x, position.y, position.z, rotation.x, rotation.y, rotation.z, scale, 0, 0, 0, false);
}
