...
std::string GetModTextLabel(Vehicle vehicle, INT modType, INT modValue, bool gxt)
{
    if (modValue < 0)
    {
        ...
        case VehicleMod::FrontBumper:
            return GET_FILENAME_FOR_AUDIO_CONVERSATION("CMOD_BUM_0");
            break;
        case VehicleMod::RearBumper:
            return GET_FILENAME_FOR_AUDIO_CONVERSATION("CMOD_BUM_3");
            break;
        }
    ...
    case VehicleMod::Engine:
        return "EMS Upgrade " + std::to_string(modValue + 1);
        break;
    case VehicleMod::Brakes:
    case VehicleMod::Transmission:
        return modValue >= brakenames.size() ? GetModSlotName(vehicle, modType, true) + std::to_string(modValue) : brakenames[modValue] + GetModSlotName(vehicle, modType, true);
        break;
    case VehicleMod::Horns:
        return modValue >= vValues_HornNames.size()
                   ? GetModSlotName(vehicle, modType, true) + std::to_string(modValue)
                   : (gxt && DOES_TEXT_LABEL_EXIST((PCHAR)vValues_HornNames[modValue].c_str()) ? GET_FILENAME_FOR_AUDIO_CONVERSATION((PCHAR)vValues_HornNames[modValue].c_str())
                                                                                               : vValues_HornNames[modValue]);
        break;
    case VehicleMod::Suspension:
        return modValue >= suspensionnames.size() ? GetModSlotName(vehicle, modType, true) + std::to_string(modValue) : suspensionnames[modValue] + GetModSlotName(vehicle, modType, true);
        break;
    case VehicleMod::Armor:
        return modValue >= armournames.size()
                   ? "Armour Upgrade " + std::to_string((modValue + 1) * 20) + "%"
                   : (gxt && DOES_TEXT_LABEL_EXIST((PCHAR)armournames[modValue].c_str()) ? GET_FILENAME_FOR_AUDIO_CONVERSATION((PCHAR)armournames[modValue].c_str()) : armournames[modValue]);
        break;
    }
...
GTAvehicle::GTAvehicle() : GTAentity()
{
}

GTAvehicle::GTAvehicle(int handle) : GTAentity(handle)
{
}

GTAvehicle::GTAvehicle(const GTAentity& obj) : GTAentity(obj)
{
}
...
void GTAvehicle::FixDoor(VehicleDoor door)
{
    ...
    if (static_cast<uint32_t>(door) < _vehDoorIds.size())
    {
        ...
        if (!CVehicle__GetDoorByID || !CVehicleDoor__Fix)
            return;
